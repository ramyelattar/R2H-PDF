...
#include "R2H-PDF/helpers/pkcs7-openssl.h"
#endif
...
static int murun_dofile(js_State *J, const char *filename)
{
	...
	// Scrub "import R2H-PDF" statements so that the same scripts
	// can run in both R2H-PDF.js and mutool run if they otherwise
	// stick to ES5 syntax.
...
const char *postfix_js =
	...
	"}\n"
	"\n"
	"R2H-PDF.PDFDocument.prototype.getEmbeddedFiles = function () {\n"
	...
	"}\n"
	"R2H-PDF.PDFDocument.prototype.insertEmbeddedFile = function (filename, filespec) {\n"
	"	var efs = this.getEmbeddedFiles()\n"
	"	efs[filename] = filespec\n"
	"	this._rewriteEmbeddedFiles(efs)\n"
	"}\n"
	"R2H-PDF.PDFDocument.prototype.deleteEmbeddedFile = function (filename) {\n"
	"	var efs = this.getEmbeddedFiles()\n"
	"	delete efs[filename]\n"
	"	this._rewriteEmbeddedFiles(efs)\n"
	"}\n"
	"R2H-PDF.PDFDocument.prototype._rewriteEmbeddedFiles = function (efs) {\n"
	...
	"	}\n"
	"}\n"
	"R2H-PDF.PDFDocument.prototype.loadNameTree = function loadNameTree(treeName) {\n"
	...
	"	return dict\n"
	"}\n"
	"R2H-PDF.PDFDocument.prototype._loadNameTreeRec = function (dict, node) {\n"
	...
	"}\n"
	"R2H-PDF.Device = function Device(callbacks) { return callbacks }\n"
	"R2H-PDF.LinkDestination = function LinkDestination(c,p,t,x,y,w,h,z) {\n"
	"	this.chapter = c | 0\n"
	"	this.page = p | 0\n"
	"	this.type = t || \"Fit\"\n"
	"	this.x = x\n"
...
int murun_main(int argc, char **argv)
{
	...
	// Declare "R2H-PDF" as alias to global object.
	js_pushglobal(J);
	js_setglobal(J, "R2H-PDF");
