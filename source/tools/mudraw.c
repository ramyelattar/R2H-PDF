...
#if FZ_ENABLE_PDF
#include "R2H-PDF/pdf.h" /* for pdf output */
#endif
...
static int convert_to_accel_path(fz_context *ctx, char outname[], char *absname, size_t len, int create)
{
	...
	if (tmpdir && create_accel_path(ctx, outname, len, create, absname, tmpdir, ".config", "R2H-PDF", NULL))
		return 1; /* OK! */
	/* TEMP and TMP are user-specific on modern windows. */
	tmpdir = getenv("TEMP");
	if (tmpdir && create_accel_path(ctx, outname, len, create, absname, tmpdir, "R2H-PDF", NULL))
		return 1; /* OK! */
	tmpdir = getenv("TMP");
	if (tmpdir && create_accel_path(ctx, outname, len, create, absname, tmpdir, "R2H-PDF", NULL))
		return 1; /* OK! */
	...
	if (tmpdir && create_accel_path(ctx, outname, len, create, absname, tmpdir, "R2H-PDF", NULL))
		return 1; /* OK! */
	tmpdir = getenv("HOME");
	if (tmpdir && create_accel_path(ctx, outname, len, create, absname, tmpdir, ".cache", "R2H-PDF", NULL))
		return 1; /* OK! */
#endif
	return 0; /* Fail */
}
