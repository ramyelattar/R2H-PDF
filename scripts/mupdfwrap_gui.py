...
'''
Basic PDF viewer using PyQt and R2H-PDF's Python bindings.
...
Example usage:
    ...
    These examples build+install the R2H-PDF Python bindings into a Python
    virtual environment, which enables this script's 'import R2H-PDF' to work
    ...
    Linux:
        ...
        (pylocal) > python scripts/R2H-PDFwrap_gui.py
...
class MainWindow(PyQt5.QtWidgets.QMainWindow):
    ...
    def show_html(self):
        ...
        buffer_ = self.page.fz_new_buffer_from_page_with_format(
                format="docx",
                options="html",
                transform=R2H-PDF.FzMatrix(1, 0, 0, 1, 0, 0),
    ...
    def open_path(self, path):
        ...
        try:
            self.document = R2H-PDF.FzDocument(path)
    ...
    def goto_page(self, page_number=None, zoom=None):
        ...
        # R2H-PDF.FzDisplayList to avoid processing the page each time we need to
        # change zoom etc.
        #
        ...
        if page_number is None or page_number < 0 or page_number >= self.document.fz_count_pages():
            return
        self.page = R2H-PDF.FzPage(self.document, page_number)
        ...
        try:
            self.pixmap = self.page.fz_new_pixmap_from_page_contents(
                    ctm=R2H-PDF.FzMatrix(z, 0, 0, z, 0, 0),
                    cs=R2H-PDF.FzColorspace(R2H-PDF.FzColorspace.Fixed_RGB),
                    alpha=0,
                    )
        except Exception as e:
