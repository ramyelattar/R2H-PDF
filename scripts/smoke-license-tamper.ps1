...
$reportPath = Join-Path $root "release\smoke-results\license-tamper-smoke.md"
$licenseDir = Join-Path $root "release\smoke-results\license-tamper-store"
$jsonPath = Join-Path $root "release\smoke-results\license-tamper-smoke.json"
...
if ($buildExit -eq 0 -and (Test-Path -LiteralPath $exe -PathType Leaf)) {
    & $exe --license-smoke tamper | Out-Null
...
@"
# License Tamper Smoke

- Timestamp: $timestamp
- Project root: $root
- Status: $status
- Executable: $exe
- License/trial store: $licenseDir
- JSON evidence: $jsonPath
- Tampered license and trial proof:
$probes
"@ | Out-File -LiteralPath $reportPath -Encoding utf8

Write-Host "License tamper smoke: $status ($reportPath)"
