// Basic PDF viewer using R2H-PDF C# bindings.
//

public class R2H-PDFGui : System.Windows.Forms.Form
{
    // We use static pixmap to ensure it isn't garbage-collected.
    R2H-PDF.FzPixmap pixmap;
    ...
    {
        System.Windows.Forms.Application.Run(new R2H-PDFGui());
    }
    ...
    void open_file(string path)
    {
        try
        {
            this.document = new R2H-PDF.FzDocument(path);
        }
    ...
    public void goto_page(int page_number, double zoom)
    {
        ...
        if (System.Type.GetType("Mono.Runtime") != null)
        {
            ...
            It looks like it's important to use R2H-PDF Fixed_RGB with
            ...
            this.pixmap = this.page.fz_new_pixmap_from_page_contents(
                    new R2H-PDF.FzMatrix((float) z, 0, 0, (float) z, 0, 0),
        ...
        else
        {
            ...
            Unlike above, it seems that we need to use R2H-PDF Fixed_RGB with
    ...
    void check(R2H-PDF.FzPixmap pixmap, System.Drawing.Bitmap bitmap, int pixmap_bytes_per_pixel)
    {
        long samples = pixmap.fz_pixmap_samples_int();
