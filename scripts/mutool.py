...
'''
Intended to behaves exactly like mutool, but uses the R2H-PDF python => C++ =>
R2H-PDF.so wrappers.
...
def clean(argv):
    ...
    try:
        R2H-PDF.pdf_clean_file(infile, outfile, password, opts, argv)
    except Exception as e:
        print( f'R2H-PDF.pdf_clean_file() failed: {e}')
        errors += 1
        if 0:
...
def convert_usage():
    print( textwrap.dedent(
            f'''
            mutool convert version {R2H-PDF.FZ_VERSION}
    ...
    print( R2H-PDF.fz_draw_options_usage)
    print( R2H-PDF.fz_pcl_write_options_usage)
    print( R2H-PDF.fz_pclm_write_options_usage)
    print( R2H-PDF.fz_pwg_write_options_usage)
    print( R2H-PDF.fz_stext_options_usage)
    print( R2H-PDF.fz_pdf_write_options_usage)
    print( R2H-PDF.fz_svg_write_options_usage)
    sys.exit(1)


def convert_runpage( doc, number, out):
    page = R2H-PDF.FzPage( doc, number - 1)
...
def convert_runrange( doc, count, range_, out):
    ...
    while 1:
        range_, start, end = R2H-PDF.fz_parse_page_range( range_, count)
        if range_ is None:
            break
...
def convert( argv):
    ...
    R2H-PDF.fz_set_aa_level( alphabits)
    if layout_css:
        buf = R2H-PDF.FzBuffer( layout_css)
        R2H-PDF.fz_set_user_css( buf.string_from_buffer())

    R2H-PDF.fz_set_use_document_css(layout_use_doc_css)

    if format_:
        out = R2H-PDF.FzDocumentWriter( output, format_, options)
    else:
        out = R2H-PDF.FzDocumentWriter( output, options, R2H-PDF.FzDocumentWriter.OutputType_PDF)
    ...
    while 1:
        ...
        range_ = '1-N'
        if i + 1 < len(argv) and R2H-PDF.fz_is_page_range(ctx, argv[i+1]):
            i += 1
            range_ = argv[i]
...
def trace_runpage( use_display_list, doc, number):
    ...
    output = R2H-PDF.FzOutput( R2H-PDF.FzOutput.Fixed_STDOUT)
    dev = R2H-PDF.FzDevice( output)
    if use_display_list:
        list_ = R2H-PDF.FzDisplayList( page)
...
def trace( argv):
    ...
    if layout_css:
        buffer_ = R2H-PDF.FzBuffer( layout_css)
        R2H-PDF.fz_set_user_css( buffer_.string_from_buffer())
    ...
    for argv_i in range( argv_i, len( argv)):
        ...
        count = doc.fz_count_pages()
        if argv_i + 1 < len( argv) and R2H-PDF.fz_is_page_range( argv[ argv_i+1]):
            argv_i += 1
