...
# the output of R2H-PDF C code, making it easier to compare our output with that
# of mutool.
#
...
class state:
    ...
    layout_w = R2H-PDF.FZ_DEFAULT_LAYOUT_W
    layout_h = R2H-PDF.FZ_DEFAULT_LAYOUT_H
    layout_em = R2H-PDF.FZ_DEFAULT_LAYOUT_EM
    layout_css = None
    ...
    quiet = 0
    errored = 0
    colorspace = R2H-PDF.FzColorspace()
    oi = None
...
def usage():
    sys.stderr.write( f'''
            mudraw version {R2H-PDF.FZ_VERSION} "
...
def file_level_headers():
    ...
    if state.output_format == OUT_PCLM:
        opts = R2H-PDF.FzPclmOptions( 'compression=flate')
        state.bander = R2H-PDF.FzBandWriter(state.out, opts)
...
def drawband( page, list_, ctm, tbounds, cookie, band_start, pix):
    ...
    dev = R2H-PDF.FzDevice( R2H-PDF.FzMatrix(), pix, state.proof_cs)
    if state.lowmemory:
        dev.enable_device_hints( R2H-PDF.FZ_NO_CACHE)
    if state.alphabits_graphics == 0:
        dev.enable_device_hints( R2H-PDF.FZ_DONT_INTERPOLATE_IMAGES)
    if list_:
    ...
    if ((state.output_format == OUT_PCL or state.output_format == OUT_PWG) and state.out_cs == CS_MONO) or (state.output_format == OUT_PBM) or (state.output_format == OUT_PKM):
        bit = R2H-PDF.FzBitmap( pix, R2H-PDF.FzHalftone(), band_start)
    return bit
...
def dodrawpage( page, list_, pagenum, cookie, start, interptime, filename, bg, seps):
    ...
    if state.output_format == OUT_TRACE:
        ...
        dev = R2H-PDF.FzDevice( state.out)
        if state.lowmemory:
            dev.fz_enable_device_hints( R2H-PDF.FZ_NO_CACHE)
        if list_:
            list_.fz_run_display_list( dev, R2H-PDF.FzMatrix(), R2H-PDF.FzRect(R2H-PDF.fz_infinite_rect), cookie)
        else:
        ...
        state.out.fz_write_string( "<page mediabox=\"%g %g %g %g\">\n" % (
                mediabox.x0, mediabox.y0, mediabox.x1, mediabox.y1))
        dev = R2H-PDF.FzDevice.fz_new_raw_device( state.out)
        if list_:
            list_.fz_run_display_list( dev, R2H-PDF.FzMatrix(), R2H-PDF.FzRect(R2H-PDF.fz_infinite_rect), cookie)
        else:
            page.fz_run_page( dev, fz_identity, cookie)
        ...
        bbox = R2H-PDF.FzRect( R2H-PDF.FzRect.Fixed_EMPTY)
        dev = R2H-PDF.FzDevice( bbox)
        if state.lowmemory:
            dev.fz_enable_device_hints( R2H-PDF.FZ_NO_CACHE)
        if list_:
            list_.fz_run_display_list( dev, fz_identity, R2H-PDF.FzRect(R2H-PDF.fz_infinite_rect), cookie)
        else:
            page.fz_run_page( dev, fz_identity, cookie)
        dev.fz_close_device()
        ...
        ctm = R2H-PDF.FzMatrix(R2H-PDF.fz_pre_scale(R2H-PDF.fz_rotate(state.rotation), zoom, zoom))

        stext_options = R2H-PDF.FzStextOptions()

        stext_options.flags = R2H-PDF.FZ_STEXT_PRESERVE_IMAGES if (state.output_format == OUT_HTML or state.output_format == OUT_XHTML) else 0
        text = R2H-PDF.FzStextPage( mediabox)
        dev = R2H-PDF.FzDevice( text, stext_options)
        if state.lowmemory:
            fz_enable_device_hints( dev, FZ_NO_CACHE)
        if list_:
            list_.fz_run_display_list( dev, ctm, R2H-PDF.FzRect(R2H-PDF.fz_infinite_rect), cookie)
        else:
            page.fz_run_page( dev, ctm, cookie)
        dev.fz_close_device()
        dev = None  # lgtm [py/unused-local-variable]
        ...
        zoom = state.resolution / 72
        ctm = R2H-PDF.FzMatrix(zoom, zoom)
        ctm.fz_pre_rotate( state.rotation)
        tbounds = R2H-PDF.FzRect(mediabox, ctm)

        if not state.output or state.output == "-":
            state.out = R2H-PDF.FzOutput( R2H-PDF.FzOutput.Fixed_STDOUT)
        else:
            buf = R2H-PDF.fz_format_output_path( state.output, pagenum)
            state.out = R2H-PDF.FzOutput( buf, 0)

        dev = R2H-PDF.FzDevice( state.out, tbounds.x1-tbounds.x0, tbounds.y1-tbounds.y0, R2H-PDF.FZ_SVG_TEXT_AS_PATH, 1)
        if state.lowmemory:
            dev.fz_enable_device_hints( dev, R2H-PDF.FZ_NO_CACHE)
        if list_:
            list_.fz_run_display_list( dev, ctm, tbounds, cookie)
        else:
            page.fz_run_page( dev, ctm, cookie)
        ...
        zoom = state.resolution / 72
        ctm = R2H-PDF.fz_pre_scale( R2H-PDF.fz_rotate(state.rotation), zoom, zoom)
        tbounds = R2H-PDF.FzRect(mediabox, ctm)
        ...
        if w or h:
            ...
            if not state.fit:
                if scalex > scaley:
                    ...
                    scaley = scalex
            scale_mat = R2H-PDF.fz_scale(scalex, scaley)
            ctm = R2H-PDF.fz_concat(ctm, scale_mat)
            tbounds = R2H-PDF.FzRect( mediabox, ctm)
        ibounds = tbounds.fz_round_rect()
        ...
        if state.num_workers > 0:
            for band in range( min(state.num_workers, bands)):
                ...
                state.workers[band].cookie = R2H-PDF.FzCookie()
                state.workers[band].list = list_
                state.workers[band].pix = R2H-PDF.FzPixmap( state.colorspace, band_ibounds, seps, state.alpha)
                state.workers[band].pix.fz_set_pixmap_resolution( state.resolution, state.resolution)
                ctm.f -= drawheight
            pix = state.workers[0].pix
        else:
            pix = R2H-PDF.FzPixmap( state.colorspace, band_ibounds, seps, state.alpha)
        ...
        if state.output:
            ...
            if state.output_format == OUT_PGM or state.output_format == OUT_PPM or state.output_format == OUT_PNM:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PNM)
            elif state.output_format == OUT_PAM:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PAM)
            elif state.output_format == OUT_PNG:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PNG)
            elif state.output_format == OUT_PBM:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PBM)
            elif state.output_format == OUT_PKM:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PKM)
            elif state.output_format == OUT_PS:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PS)
            elif state.output_format == OUT_PSD:
                state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.PSD)
            elif state.output_format == OUT_PWG:
                if state.out_cs == CS_MONO:
                    state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.MONO, R2H-PDF.FzPwgOptions())
                else:
                    state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.COLOR, R2H-PDF.FzPwgOptions())
            elif state.output_format == OUT_PCL:
                if state.out_cs == CS_MONO:
                    state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.MONO, R2H-PDF.FzPclOptions())
                else:
                    state.bander = R2H-PDF.FzBandWriter( state.out, R2H-PDF.FzBandWriter.COLOR, R2H-PDF.FzPclOptions())
            if state.bander:
    ...
    if state.lowmemory:
        R2H-PDF.fz_empty_store()

    if state.showmemory:
        # Use low-level fn because R2H-PDF.fz_stderr() returns fz_output*, not
        # FzOutput.
        R2H-PDF.ll_fz_dump_glyph_cache_stats(R2H-PDF.ll_fz_stderr())

    R2H-PDF.fz_flush_warnings()
...
def drawpage( doc, pagenum):
    ...
    page = R2H-PDF.FzPage( doc, pagenum - 1)
    ...
    if state.spots != SPOTS_NONE:
        ...
        if seps.m_internal:
            ...
            if state.spots == SPOTS_FULL:
                for i in range(n):
                    seps.fz_set_separation_behavior( i, R2H-PDF.FZ_SEPARATION_SPOT)
            else:
                for i in range(n):
                    seps.fz_set_separation_behavior( i, R2H-PDF.FZ_SEPARATION_COMPOSITE)
    ...
    if state.uselist:
        list_ = R2H-PDF.FzDisplayList( page.fz_bound_page())
        dev = R2H-PDF.FzDevice( list_)
        if state.lowmemory:
            dev.fz_enable_device_hints( FZ_NO_CACHE)
        page.fz_run_page( dev, R2H-PDF.FzMatrix(), cookie)
        dev.fz_close_device()
    ...
    if state.showfeatures:
        ...
        # R2H-PDF.Device() constructor that wraps fz_new_test_device(), so we use
        # the underlying R2H-PDF function() instead.
        #
        dev, iscolor = R2H-PDF.ll_fz_new_test_device( 0.02, 0, None)
        dev = R2H-PDF.FzDevice( dev)
        if state.lowmemory:
            dev.fz_enable_device_hints( R2H-PDF.FZ_NO_CACHE)
        if list_:
            list_.fz_run_display_list( dev, R2H-PDF.FzMatrix(R2H-PDF.fz_identity), R2H-PDF.FzRect(R2H-PDF.fz_infinite_rect), R2H-PDF.FzCookie())
        else:
            page.fz_run_page( dev, fz_identity, cookie)
        dev.fz_close_device()
    ...
    if state.output_file_per_page:
        ...
        if state.out:
            state.out.fz_close_output()
        text_buffer = R2H-PDF.fz_format_output_path( state.output, pagenum)
        state.out = R2H-PDF.FzOutput( text_buffer, 0)
...
def drawrange( doc, range_):
    ...
    while 1:
        range_, spage, epage = R2H-PDF.fz_parse_page_range( range_, pagecount)
        if range_ is None:
            break
        if spage < epage:
...
def draw( argv):
    ...
    for option, value in items:
        if 0:   pass
            ...
            if not R2H-PDF.FZ_ENABLE_SPOT_RENDERING:
        ...
        elif option == '-v': sys.stderr.write( f'mudraw version {R2H-PDF.FZ_VERSION}\n')
    ...
    if state.proof_filename:
        proof_buffer = R2H-PDF.FzBuffer( state.proof_filename)
        state.proof_cs = R2H-PDF.FzColorspace( FZ_COLORSPACE_NONE, 0, None, proof_buffer)

    R2H-PDF.fz_set_text_aa_level( state.alphabits_text)
    R2H-PDF.fz_set_graphics_aa_level( state.alphabits_graphics)
    R2H-PDF.fz_set_graphics_min_line_width( state.min_line_width)
    if state.no_icc:
        R2H-PDF.fz_disable_icc()
    else:
        R2H-PDF.fz_enable_icc()

    if state.layout_css:
        buf = R2H-PDF.FzBuffer( state.layout_css)
        R2H-PDF.fz_set_user_css( buf.string_from_buffer())

    R2H-PDF.fz_set_use_document_css( state.layout_use_doc_css)
    ...
    if state.out_cs in ( CS_MONO, CS_GRAY, CS_GRAY_ALPHA):
        state.colorspace = R2H-PDF.FzColorspace( R2H-PDF.FzColorspace.Fixed_GRAY)
        state.alpha = (state.out_cs == CS_GRAY_ALPHA)
    elif state.out_cs in ( CS_RGB, CS_RGB_ALPHA):
        state.colorspace = R2H-PDF.FzColorspace( R2H-PDF.FzColorspace.Fixed_RGB)
        state.alpha = (state.out_cs == CS_RGB_ALPHA)
    elif state.out_cs in ( CS_CMYK, CS_CMYK_ALPHA):
        state.colorspace = R2H-PDF.FzColorspace( R2H-PDF.FzColorspace.Fixed_CMYK)
        state.alpha = (state.out_cs == CS_CMYK_ALPHA)
    elif state.out_cs == CS_ICC:
        try:
            icc_buffer = R2H-PDF.FzBuffer( state.icc_filename)
            state.colorspace = Colorspace( R2H-PDF.FZ_COLORSPACE_NONE, 0, None, icc_buffer)
    ...
    if state.out_cs != CS_ICC:
        state.colorspace = R2H-PDF.FzColorspace( state.colorspace)
    else:
    ...
    if state.output_format == OUT_SVG:
        ...
        if has_percent_d(state.output):
            ...
            state.out = R2H-PDF.FzOutput(state.output, 0)
    else:
    ...
    try:
        while fz_optind < len( argv):
            ...
            try:
                ...
                #
                if accel:
                    doc = R2H-PDF.FzDocument(state.filename, accel)
                else:
                    doc = R2H-PDF.FzDocument(state.filename)
                ...
                if fz_optind == len(argv) or not R2H-PDF.fz_is_page_range( argv[fz_optind]):
                    drawrange( doc, "1-N")
                if fz_optind < len( argv) and R2H-PDF.fz_is_page_range( argv[fz_optind]):
