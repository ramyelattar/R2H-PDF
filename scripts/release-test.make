...
test-java-examples: make-java-build platform/java/pdfref17.pdf
	/usr/bin/test 93ecea2a290429260f0fef4107012a51 == $$(R2H-PDF_ARGS="pdfref17.pdf 1140" $(MAKE) -C platform/java build=release run-example | grep -Ev '^(make|java)' | md5sum - | cut -d' ' -f1)
...
test-java-build: make-java-build
	R2H-PDF_ARGS=pdfref17.pdf $(MAKE) -C platform/java build=release run
nuke:
	$(MAKE) nuke
