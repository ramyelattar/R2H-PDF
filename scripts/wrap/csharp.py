...
def make_outparam_helper_csharp(
    ...
    '''
    Write C# code for a convenient tuple-returning wrapper for R2H-PDF
    ...
    if fnname == 'fz_buffer_extract':
        ...
        write(
                ...
                public static class R2H-PDF_{rename.class_('fz_buffer')}_extract
                {{
                    public static byte[] {rename.method('fz_buffer', 'fz_buffer_extract')}(this R2H-PDF.{rename.class_('fz_buffer')} buffer)
                    {{
                        var outparams = new R2H-PDF.{rename.ll_fn('fz_buffer_storage')}_outparams();
                        uint n = R2H-PDF.R2H-PDF.{rename.ll_fn('fz_buffer_storage')}_outparams_fn(buffer.m_internal, outparams);
                        var raw1 = R2H-PDF.SWIGTYPE_p_unsigned_char.getCPtr(outparams.datap);
    ...
    write(f'\n')
    write(f'// Out-params extension method for C# class {rename.class_(arg0.alt.type.spelling)} (wrapper for R2H-PDF struct {arg0.alt.type.spelling}),\n')
    write(f'// adding class method {method_name}() (wrapper for {fnname}())\n')
    write(f'// which returns out-params directly.\n')
    write(f'//\n')
    write(f'public static class R2H-PDF_{main_name}_outparams_helper\n')
    write(f'{{\n')
    write(f'    public static ')

    def write_type(alt, type_):
        if alt:
            write(f'R2H-PDF.{rename.class_(alt.type.spelling)}')
        elif parse.is_pointer_to(type_, 'char'):
            write( f'string')
        else:
            text = cpp.declaration_text(type_, '', top_level='R2H-PDF.').strip()
    ...
    for arg in parse.get_args( tu, cursor):
        ...
        if arg.alt:
            # E.g. 'Document doc'.
            write(f'R2H-PDF.{rename.class_(arg.alt.type.spelling)} {arg.name_csharp}')
        elif parse.is_pointer_to(arg.cursor.type, 'char'):
            write(f'string {arg.name_csharp}')
        else:
            text = cpp.declaration_text(arg.cursor.type, arg.name_csharp, top_level='R2H-PDF.').strip()
    ...
    write(f'        var outparams = new R2H-PDF.{main_name}_outparams();\n')
    write(f'        ')

    # Generate function call.
    #
    # The C# *_outparams_fn() generated by swig is inside namespace R2H-PDF {
    # class R2H-PDF { ... } }, so we access it using the rather clumsy prefix
    # 'R2H-PDF.R2H-PDF.'. It will have been generated from a C++ function
    # (generate by us) which is in top-level namespace R2H-PDF, but swig
    # appears to generate the same code even if the C++ function is not in
    # a namespace.
    #
    if not return_void:
        write(f'var ret = ')
    write(f'R2H-PDF.R2H-PDF.{main_name}_outparams_fn(')
    sep = ''
    for arg in parse.get_args( tu, cursor):
        if arg.out_param:
            continue
    ...
    if not return_void:
        if return_alt:
            write(f'new R2H-PDF.{rename.class_(return_alt.type.spelling)}(ret)')
        else:
            write(f'ret')
        sep = ', '
    for arg in parse.get_args( tu, cursor):
        if arg.out_param:
            ...
            if arg.alt:
                    write(f'new R2H-PDF.{rename.class_(arg.alt.type.spelling)}(outparams.{arg.name_csharp})')
...
def csharp_settings(build_dirs):
    '''
    Returns (csc, mono, R2H-PDF_cs).

    csc: C# compiler.
    mono: C# interpreter ("" on Windows).
    R2H-PDF_cs: R2H-PDF C# code.

    `R2H-PDF_cs` will be None if `build_dirs` is false.
    ...
    #   mono:build/shared-release/libR2H-PDFcpp.so: undefined symbol '_ZdlPv'
    ...
    if build_dirs:
        R2H-PDF_cs = os.path.relpath(f'{build_dirs.dir_so}/R2H-PDF.cs')
    else:
        R2H-PDF_cs = None
    return csc, mono, R2H-PDF_cs
