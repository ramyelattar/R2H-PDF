...
try:
    ...
    if '--venv' not in sys.argv:
        jlib.log('Warning: failed to import clang.cindex: {e=}\n'
                f'We need Clang Python to build R2H-PDF python.\n'
...
omit_fns = [
        'fz_open_file_w',
        'fz_colorspace_name_process_colorants', # Not implemented in R2H-PDF.so?
        'fz_clone_context_internal',            # Not implemented in R2H-PDF?
...
class BuildDirs:
    ...
    def __init__( self):
        ...
        # Assume we are in R2H-PDF/scripts/.
        ...
        dir_R2H-PDF = abspath( f'{file_}/../../../')
        assert not dir_R2H-PDF.endswith( '/')

        # Directories used with --build.
        self.dir_R2H-PDF = dir_R2H-PDF

        # Directory used with --ref.
        self.ref_dir = abspath( f'{self.dir_R2H-PDF}/R2H-PDFwrap_ref')
        assert not self.ref_dir.endswith( '/')

        self.set_dir_so( f'{self.dir_R2H-PDF}/build/shared-release')
    ...
    def set_dir_so( self, dir_so):
        ...
        self.R2H-PDFcpp_swig_i    = lambda language: f'{self.dir_R2H-PDF}/platform/{language}/R2H-PDFcpp_swig{Py_LIMITED_API_infix}.i'
        self.R2H-PDFcpp_swig_cpp  = lambda language: self.R2H-PDFcpp_swig_i(language) + '.cpp'
