'''
Extra information to customise how we wrap R2H-PDF structs into C++ classes.
'''
...
class ClassExtra:
    '''
    Extra methods/features when wrapping a particular R2H-PDF struct into a C++
    class.
    '''
    def __init__( self,
            accessors=None,
        ...
        virtual_fnptrs:
            If true, should be a dict with these keys:
                ...
                alloc:
                    ...
                    If the wrapper class is a POD, the R2H-PDF struct is already
                    ...
                    instance of the R2H-PDF struct.

                    Should typically set up the R2H-PDF struct so that `self_()`
            ...
            to be overridden in Python/C#. Thus one can make R2H-PDF function
...
#
# We use R2H-PDF struct names as keys.
#
classextras = ClassExtras(
        ...
        fz_band_writer = ClassExtra(
                class_top = '''
                    /* We use these enums to support construction via all the relevant R2H-PDF functions. */
        ...
        fz_colorspace = ClassExtra(
                ...
                constructor_raw=1,
                class_top = '''
                        /* We use this enums to support construction via all the relevant R2H-PDF functions. */
                        enum Fixed
                        {
                            Fixed_GRAY,
                            Fixed_RGB,
        ...
        fz_path_walker = ClassExtra(
                ...
                virtual_fnptrs = dict(
                    ...
                    comment = textwrap.dedent(f'''
                            ...
                            class-aware wrapper R2H-PDF::fz_walk_path() should be
                            called like:

                                R2H-PDF.FzPath path = ...;
                                struct Walker : R2H-PDF.FzPathWalker2 {...};
                                Walker walker(...);
                                R2H-PDF::fz_walk_path(path, walker, walker.m_internal);
                            */
                            ''')
                    ),
                ),
