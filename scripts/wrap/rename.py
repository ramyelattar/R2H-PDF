'''
Functions to create C++ names from R2H-PDF names.
'''
...
def snake_to_camel( name, initial):
    ...
    fooBarQ_A
    '''
    # libclang can treat size_t oddly, which we work around when parsing R2H-PDF
...
#
# R2H-PDFcpp_swig.cpp: In function PyObject* _wrap_vthrow_fn(PyObject*, PyObject*)
# R2H-PDFcpp_swig.cpp:88571:15: error: invalid array assignment
#        arg3 = *temp;
...
def error_class( error_enum):
    '''
    Name of generated class for R2H-PDF `FZ_ERROR_*` enum.
    '''
...
def c_fn( fnname):
    '''
    Returns fully-qualified name of R2H-PDF C function `fnname()`.
    '''
    return f'::{fnname}'

def ll_fn( fnname):
    '''
    Returns name of low-level wrapper function for R2H-PDF C function `fnname()`,
    adding a `ctx` arg and converting R2H-PDF exceptions into C++ exceptions.
    '''
...
def namespace_ll_fn( fnname):
    '''
    Returns full-qualified name of low-level wrapper function for R2H-PDF C
    function `fnname()`, adding a `ctx` arg and converting R2H-PDF exceptions
    into C++ exceptions.
    '''
...
def fn( fnname):
    '''
    Returns name of wrapper function for R2H-PDF C function `fnname()`, using
...
def namespace_fn( fnname):
    '''
    Returns fully-qualified name of wrapper function for R2H-PDF C function
...
def class_( structname):
    '''
    Returns name of class that wraps R2H-PDF struct `structname`.
    '''
...
def namespace_class( structname):
    '''
    Returns fully-qualified name of class that wraps R2H-PDF struct `structname`.
    '''
    return f'{namespace()}::{class_(structname)}'

def method( structname, fnname):
    '''
    Returns name of class method that wraps R2H-PDF function `fnname()`.
    '''
    return fnname
    if structname:
