...
def translate_ucdn_macros( build_dirs):
    ...
    '''
    out = io.StringIO()
    with open( f'{build_dirs.dir_R2H-PDF}/include/R2H-PDF/ucdn.h') as f:
        text = f.read()
    out.write( '\n')
    out.write( '\n')
    out.write( 'enum\n')
...
def _csharp_unicode_prefix():
    ...
    into utf8 when calling R2H-PDF, and convert strings returned by R2H-PDF (which
    are utf8) into utf16.
...
def build_swig(
    ...
    '''
    Builds python or C# wrappers for all R2H-PDF_* functions and classes, by
    ...
    common = textwrap.dedent(f'''
            ...
            #include "R2H-PDF/internal.h"
            #include "R2H-PDF/exceptions.h"
            #include "R2H-PDF/extra.h"

            #ifdef NDEBUG
                static bool g_R2H-PDF_trace_director = false;
                static bool g_R2H-PDF_trace_exceptions = false;
            #else
                static bool g_R2H-PDF_trace_director = R2H-PDF::internal_env_flag("R2H-PDF_trace_director");
                static bool g_R2H-PDF_trace_exceptions = R2H-PDF::internal_env_flag("R2H-PDF_trace_exceptions");
            #endif
    ...
    if language == 'python':
        common += textwrap.dedent(f'''
                ...
                For example to create a R2H-PDF fz_buffer* from a copy of a
                Python bytes instance:
                    bs = b'qwerty'
                    buffer_ = R2H-PDF.fz_new_buffer_from_copied_data(R2H-PDF.python_buffer_data(bs), len(bs))
                */
                ...
                /* R2H-PDFpy optimisation for copying raw data into pixmap. `samples` must
    ...
    common += textwrap.dedent(f'''
            ...
            {{
                ...
                {{
                    ...
                    {{
                        R2H-PDF::ll_fz_set_warning_callback( s_print, this);
                    }}
                    else if (m_description == "error")
                    {{
                        R2H-PDF::ll_fz_set_error_callback( s_print, this);
                    }}
                    else
                    {{
                        std::cerr
            ...
            {{
                ...
                {{
                    ...
                    {{
                        ...
                        //R2H-PDF::fz_warn( "skipping widget annotation");
                        continue;
                    }}
    ...
    text += '%module(directors="1") R2H-PDF\n'
    ...
    %feature("director:except")
    {
        if ($error != NULL)
        {
            ...
            1. Python code calls a function in the Python `R2H-PDF` module.
            2. - which calls SWIG C++ code.
            3. - which calls R2H-PDF C++ API wrapper function.
            4. - which calls R2H-PDF C code which calls an R2H-PDF struct's function pointer.
            5. - which calls R2H-PDF C++ API Director wrapper (e.g. R2H-PDF::FzDevice2) virtual function.
            ...
            5. R2H-PDF C++ API Director wrapper converts the C++ exception into a R2H-PDF fz_try/catch exception.
            4. R2H-PDF C code allows the exception to propagate or catches and rethrows or throws a new fz_try/catch exception.
            3. R2H-PDF C++ API wrapper function converts the fz_try/catch exception into a C++ exception.
            ...
            first C++ exception propagate directly through R2H-PDF C code without
            being a fz_try/catch exception, because it would mess up R2H-PDF C
            code's fz_try/catch exception stack.

            Unfortuntately R2H-PDF fz_try/catch exception strings are limited to
            ...
            */
            ...
            if (g_R2H-PDF_trace_director)
            {
                ...
                std::cerr << "========\\n";
                std::cerr << "g_R2H-PDF_trace_director set: Converting Python error into C++ exception:" << "\\n";
                #ifndef _WIN32
            ...
            if (traceback)
            {
                ...
                /* Use exception_info() (copied from R2H-PDF/scripts/jlib.py) to get
                detailed backtrace. */
                if (1)
                {
    ...
    # Ignore all C R2H-PDF functions; SWIG will still look at the C++ API in
    # namespace mudf.
    ...
    text += textwrap.dedent(f'''
            // Not implemented in R2H-PDF.so: fz_colorspace_name_process_colorants
            ...
            // Might prefer to #include R2H-PDF/exceptions.h and make the
            // %exception block below handle all the different exception types,
            // but swig-3 cannot parse 'throw()' in R2H-PDF/exceptions.h.
            //
    ...
    if language == 'python':
        ...
        text += textwrap.dedent(f'''
                ...
                def pdf_dict_putl_drop(obj, *tail):
                    raise Exception('R2H-PDF.pdf_dict_putl_drop() is unsupported and unnecessary in Python because reference counting is automatic. Instead use R2H-PDF.pdf_dict_putl().')
                ...
                # Override R2H-PDF_convert_color() to return (rgb0, rgb1, rgb2, rgb3).
        ...
        #
        # E.g. this allows Python code to print a R2H-PDF.fz_rect instance.
        #
        ...
        #
        # E.g. this allows Python code to print a R2H-PDF.Rect instance.
        #
        for struct_name in generated.to_string_structnames:
    ...
    if 1:   # lgtm [py/constant-conditional-expression]
        ...
        # include/R2H-PDF/pdf/object.h defines an enum which contains a #include.
        #
        ...
        #
        # So instead we copy include/R2H-PDF/pdf/object.h into
        # {build_dirs.dir_R2H-PDF}/platform/python/include/R2H-PDF/pdf/object.h,
        # manually expanding the #include using a Python .replace() call. Then
        # we specify {build_dirs.dir_R2H-PDF}/platform/python/include as the
        # first include path so that our modified R2H-PDF/pdf/object.h will get
        # included in preference to the original.
        #
        os.makedirs(f'{build_dirs.dir_R2H-PDF}/platform/python/include/R2H-PDF/pdf', exist_ok=True)
        with open( f'{build_dirs.dir_R2H-PDF}/include/R2H-PDF/pdf/object.h') as f:
            o = f.read()
        with open( f'{build_dirs.dir_R2H-PDF}/include/R2H-PDF/pdf/name-table.h') as f:
            name_table_h = f.read()
        oo = o.replace( '#include "R2H-PDF/pdf/name-table.h"\n', name_table_h)
        assert oo != o
        jlib.fs_update( oo, f'{build_dirs.dir_R2H-PDF}/platform/python/include/R2H-PDF/pdf/object.h')

    swig_i      = build_dirs.R2H-PDFcpp_swig_i(language)
    swig_cpp    = build_dirs.R2H-PDFcpp_swig_cpp(language)
    include1    = f'{build_dirs.dir_R2H-PDF}/include/'
    include2    = f'{build_dirs.dir_R2H-PDF}/platform/c++/include'
    swig_py     = f'{build_dirs.dir_so}/R2H-PDF.py'

    swig2_i     = f'{build_dirs.dir_R2H-PDF}/platform/{language}/R2H-PDFcpp2_swig.i'
    swig2_cpp   = f'{build_dirs.dir_R2H-PDF}/platform/{language}/R2H-PDFcpp2_swig.cpp'
    swig2_py    = f'{build_dirs.dir_so}/R2H-PDF2.py'

    os.makedirs( f'{build_dirs.dir_R2H-PDF}/platform/{language}', exist_ok=True)
    ...
    disable_swig_warnings = [
            ...
            512,    # Warning 512: Overloaded method R2H-PDF::DrawOptions::internal() const ignored, using non-const method R2H-PDF::DrawOptions::internal() instead.
            509,    # Warning 509: Overloaded method R2H-PDF::FzAaContext::FzAaContext(::fz_aa_context const) effectively ignored,
    ...
    # unnecessary rebuilds, e.g. if .cpp is older than a R2H-PDF header.
    #
    swig_cpp_old = None
    ...
    if language == 'python':
        ...
        def make_command( module, cpp, swig_i):
            ...
            # We need to predefine R2H-PDF_FITZ_HEAP_H to disable parsing of
            # include/R2H-PDF/fitz/heap.h. Otherwise swig's preprocessor seems to
            # ignore #undef's in include/R2H-PDF/fitz/heap-imp.h then complains
            # about redefinition of macros in include/R2H-PDF/fitz/heap.h.
            command = f'''
                    "{swig_command}"
                        ...
                        {disable_swig_warnings}
                        -module {module}
                        -outdir {os.path.relpath(build_dirs.dir_R2H-PDF)}/platform/python
                        -o {cpp}
                        -includeall
                        {os.environ.get('XCXXFLAGS', '')}
                        -I{os.path.relpath(build_dirs.dir_R2H-PDF)}/platform/python/include
                        -I{os.path.relpath(include1)}
                        -I{os.path.relpath(include2)}
                        -ignoremissing
                        -DR2H-PDF_FITZ_HEAP_H
                        {swig_i}
                    '''
            return command
        ...
        def modify_cpp(path_in, path_out):
            ...
            # to
            # PyUnicode_DecodeUTF8(str, len, "surrogateescape_R2H-PDF")
            #
            b = a.replace('"surrogateescape"', '"surrogateescape_R2H-PDF"')
            assert b != a
        ...
        # Make main R2H-PDF .so. We use intermediate .cpp and .py filenames to
        # allow us to post-process them.
        swig_cpp_ = f'{build_dirs.dir_R2H-PDF}/platform/python/R2H-PDF.intermediate.cpp'
        swig_py_ = f'{build_dirs.dir_R2H-PDF}/platform/python/R2H-PDF.py'
        command = make_command( 'R2H-PDF', swig_cpp_, swig_i)
        rebuilt = jlib.build(
        ...
        outdir = os.path.relpath(f'{build_dirs.dir_R2H-PDF}/platform/csharp')
        os.makedirs(outdir, exist_ok=True)
        # Looks like swig comes up with 'R2H-PDFcpp_swig_wrap.cxx' leafname.
        #
        # We include platform/python/include in order to pick up the modified
        # include/R2H-PDF/pdf/object.h that we generate elsewhere.
        dllimport = 'R2H-PDFcsharp.so'
        if state_.windows:
            # Would like to specify relative path to .dll with:
            #   dllimport = os.path.relpath( f'{build_dirs.dir_so}/R2H-PDFcsharp.dll')
        ...
        jlib.log1( 'Updating cs2 => {build_dirs.dir_so}/R2H-PDF.cs')
        jlib.fs_update(cs2, f'{build_dirs.dir_so}/R2H-PDF.cs')
        #jlib.fs_copy(f'{outdir}/R2H-PDF.cs', f'{build_dirs.dir_so}/R2H-PDF.cs')
        jlib.log1('{rebuilt=}')
...
def test_swig_charp_exceptions(x32):
    ...
    # the Python bindings and convert each of the various R2H-PDF C++ exception
