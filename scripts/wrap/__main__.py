...
r'''
Support for generating C++ and python wrappers for the R2H-PDF API.
...
C++ wrapping:
    ...
    Namespaces:
        ...
        All generated functions and classes are in the 'R2H-PDF' namespace.

    Wrapper classes:

        For each R2H-PDF C struct, we provide a wrapper class with a CamelCase
        version of the struct name, e.g. the wrapper for fz_display_list is
        R2H-PDF::FzDisplayList.
        ...
        Member functions:
            ...
            Member functions are provided which wrap all relevant R2H-PDF C
            ...
            instead of pointers to R2H-PDF C structs, and similarly return
            wrapper classes by value instead of returning a pointer to a R2H-PDF
            C struct.
        ...
        Reference counting:
            ...
            Raw constructors that take a pointer to an underlying R2H-PDF struct
            do not call `fz_keep_*()` - it is expected that any supplied R2H-PDF
            ...
            Debugging reference counting:
                ...
                If environmental variable R2H-PDF_check_refs is "1", we do
                runtime checks of the generated code's handling of structs that
                have a reference count (i.e. they have a `int refs;` member).

                If the number of wrapper class instances for a particular R2H-PDF
    ...
    Wrapper functions:
        ...
        Class-aware wrappers:
            ...
            We provide a class-aware wrapper for each R2H-PDF C function; these
            have the same name as the R2H-PDF C function and are identical to
            the corresponding class member function except that they take an
            explicit first arg instead of the implicit C++ `this`.

        Low-level wrappers:

            We provide a low-level wrapper for each C R2H-PDF function; these
    ...
    Text representation of POD data:
        ...
        For selected POD R2H-PDF structs, we provide functions that give a
        ...
        Text representation of a R2H-PDF POD C struct:
    ...
    Examples:
        ...
        generated code:
            ...
            R2H-PDF_trace
                ...
                a R2H-PDF function, showing the args.

            R2H-PDF_trace_director
                If "1", generated code outputs a diagnostic when doing special
                handling of R2H-PDF structs containing function pointers.

            R2H-PDF_trace_exceptions
                If "1", generated code outputs diagnostics when we catch a
                R2H-PDF setjmp/longjmp exception and convert it into a C++
                exception.

            R2H-PDF_check_refs
                If "1", generated code checks R2H-PDF struct reference counts at
                runtime. See below for details.

    Details:

        We use clang-python to parse the R2H-PDF header files, and generate C++
        headers and source code that gives wrappers for all R2H-PDF functions.

        We also generate C++ classes that wrap all R2H-PDF structs, adding in
        various constructors and methods that wrap auto-detected R2H-PDF C
        functions, plus explicitly-specified methods that wrap/use R2H-PDF C
        functions.
        ...
        More specifically, for each wrapper class:
            ...
            Constructors:
                ...
                We look for all R2H-PDF functions called `fz_new_*()` or
            ...
            Methods:
                ...
                We look for all R2H-PDF functions that take the wrapped struct as
            ...
            Other:
                ...
                There are various subleties with wrapper classes for R2H-PDF
                structs that are not copyable etc.

        Internal `fz_context*`'s:

            `R2H-PDF::*` functions and methods generally have the same args
            as the R2H-PDF functions that they wrap except that they don't
        ...
        Extra items:
            ...
            `R2H-PDF::metadata_keys`: This is a global const vector of
        ...
        Output parameters:
            ...
            Binary out-param data:
                ...
                Some R2H-PDF functions return binary data, typically with an
...
Python wrapping:
    ...
    We generate a Python module called `R2H-PDF` which directly wraps the C++ API,
    ...
    Special handing if `fz_buffer` data:
        ...
        Generic data access:
            ...
            `R2H-PDF.python_buffer_data(b: bytes)`:
                Returns SWIG proxy for an `unsigned char*` that points to
                `<b>`'s data.

            `R2H-PDF.raw_to_python_bytes(data, size):`
        ...
        Wrappers for `fz_buffer_storage()`:
            ...
            `R2H-PDF.raw_to_python_bytes(data, size)` to get a Python `bytes`
        ...
        Wrappers for `fz_new_buffer_from_copied_data()`:
            ...
            One can create an R2H-PDF buffer that contains a copy of a Python
            `bytes` by using the special `R2H-PDF.python_buffer_data()`
            function. This returns a SWIG proxy for an `unsigned char*` that
            points to the `bytes` instance's data:

                ```
                bs = b'qwerty'
                buffer_ = R2H-PDF.new_buffer_from_copied_data(R2H-PDF.python_buffer_data(bs), len(bs))
                ```
...
Tools required to build:
    ...
    Clang:
        ...
        Clang versions:
            ...
            so wrappers for R2H-PDF functions are omitted from the generated C++
            code.
...
Building Python bindings:
    ...
    Build and install the R2H-PDF Python bindings as module `R2H-PDF` in a Python
    virtual environment, using R2H-PDF's `setup.py` script:
        ...
        Use the R2H-PDF module:
            (pylocal) > python
            >>> import R2H-PDF
            >>>

    Build R2H-PDF Python bindings without a Python virtual environment, using
    scripts/R2H-PDFwrap.py:
        ...
        Build and test:
            > ./scripts/R2H-PDFwrap.py -d build/shared-release -b all --test-python

        Use the R2H-PDF module by setting PYTHONPATH:
...
Building C# bindings:
    ...
    Build R2H-PDF C# bindings using scripts/R2H-PDFwrap.py:
        ...
        Build and test:
            > ./scripts/R2H-PDFwrap.py -d build/shared-release -b --csharp all --test-csharp
...
Windows builds:
    ...
    Required predefined macros:
        ...
        Code that will use the R2H-PDF DLL must be built with FZ_DLL_CLIENT
        predefined.

        The R2H-PDF DLL itself is built with FZ_DLL predefined.

    DLLs:

        There is no separate C library, instead the C and C++ API are
        both in R2H-PDFcpp.dll, which is built by running devenv on
        platform/win32/R2H-PDF.sln.

        The Python SWIG library is called _R2H-PDF.pyd which,
        despite the name, is a standard Windows DLL, built from
        platform/python/R2H-PDFcpp_swig.i.cpp.

    DLL export of functions and data:

        On Windows, include/R2H-PDF/fitz/export.h defines FZ_FUNCTION and FZ_DATA
        to __declspec(dllexport) and/or __declspec(dllimport) depending on
        whether FZ_DLL or FZ_DLL_CLIENT are defined.

        All R2H-PDF headers prefix declarations of public global data with
        FZ_DATA.

        All generated C++ code prefixes functions with FZ_FUNCTION and data
        with FZ_DATA.

        When building R2H-PDFcpp.dll on Windows we link with the auto-generated
        platform/c++/windows_R2H-PDF.def file; this lists all C public global
        data.
        ...
        otherwise we get unresolved symbols when building R2H-PDF client code.
    ...
    Building the DLLs:
        ...
        Building _R2H-PDF.pyd is tricky because it needs to be built with a
        specific Python.h and linked with a specific python.lib. This is done
        by setting environmental variables R2H-PDF_PYTHON_INCLUDE_PATH and
        R2H-PDF_PYTHON_LIBRARY_PATH when running devenv.com, which are referenced
        by platform/win32/R2H-PDFpyswig.vcxproj. Thus one cannot easily build
        _R2H-PDF.pyd directly from the Visual Studio GUI.

        [In the git history there is code that builds _R2H-PDF.pyd by running the
...
Usage:
    ...
    Args:
        ...
        --build [<args>] <actions>:
            ...
            args:
                ...
                --m-target <target>
                    ...
                    muraster.exe as well as R2H-PDFcpp64.dll.
                --m-vars <text>
                    Text to insert near start of the action 'm' make command,
                    typically to set R2H-PDF build flags, for example:
            ...
            order. If <actions> is 'all', it is replaced by m0123.
                ...
                m:
                    Builds libR2H-PDF.so by running make in the R2H-PDF/
                ...
                0:
                    ...
                    R2H-PDF/platform/c++/{implementation,include}.

                1:
                    Compile and link source files created by action=0.

                    Generates:
                        <dir-so>/libR2H-PDFcpp.so

                    This gives a C++ interface onto R2H-PDF.
                ...
                2:
                    ...
                    For example for Python this generates:
                        ...
                        R2H-PDF/platform/python/R2H-PDFcpp_swig.i
                        R2H-PDF/platform/python/R2H-PDFcpp_swig.i.cpp
                        R2H-PDF/build/shared-release/R2H-PDF.py

                    Note that this requires action=0 to have been run previously.

                3:
                    Compile and links the R2H-PDFcpp_swig.i.cpp file created by
                    action=2. Requires libR2H-PDF.so to be available, e.g. built by
                    the --libR2H-PDF.so option.

                    For example for Python this generates:

                        R2H-PDF/build/shared-release/_R2H-PDF.so

                    Along with R2H-PDF/platform/python/R2H-PDF.py (generated by
                    action=2), this implements the R2H-PDF python module.
        ...
        --check-headers [-k] <which>
            ...
            which:
                If 'all', we run on all headers in .../R2H-PDF/include. Otherwise
                if <which> ends with '+', we run on all remaining headers in
                .../R2H-PDF/include starting with <which>. Otherwise the name of
                header to test.
        ...
        --diff
            Compares generated files with those in the R2H-PDFwrap_ref/ directory
        ...
        --dir-so <directory>
            ...
            it must match what was used when libR2H-PDF.so was built).
        ...
        --make <make-command>
            ...
            If not specified, we use $R2H-PDF_MAKE. If this is not set, we use
        ...
        --ref
            Copy generated C++ files to R2H-PDFwrap_ref/ directory for use by --diff.
        ...
        --swig <swig>
            ...
            over doxygen-style comments into R2H-PDF.py. Otherwise we use
            '%feature("autodoc", "3");' to generate comments with type information
            for args in R2H-PDF.py. [These two don't seem to be usable at the same
            time in swig-4.]
        ...
        --test-python-fitz [<options>] all|iter|<script-name>
            Tests fitz.py with PyR2H-PDF. Requires 'pkg_add py3-test' or similar.
            options:
                Passed to py.test-3.
        ...
        --test-setup.py <arg>
            Tests that setup.py installs a usable Python R2H-PDF module.
                ...
                * Runs scripts/R2H-PDFwrap_test.py.
                    * Imports R2H-PDF and checks basic functionality.
                * Deactivates the Python environment.

        --venv
            If specified, should be the first arg in the command line.

            Re-runs R2H-PDFwrap.py in a Python venv containing libclang
        ...
        --windows-cmd ...
            Runs R2H-PDFwrap.py via cmd.exe, passing remaining args. Useful to
    ...
    Examples:
        ...
        ./scripts/R2H-PDFwrap.py -b all -t
            Build all (release build) and test.

        ./scripts/R2H-PDFwrap.py -d build/shared-debug -b all -t
            Build all (debug build) and test.

        ./scripts/R2H-PDFwrap.py -b 0 --compare-fz_usage platform/gl
            Compare generated class methods with functions called by platform/gl
            code.

        python3 -m cProfile -s cumulative ./scripts/R2H-PDFwrap.py --venv -b 0
            Profile generation of C++ source code.

        ./scripts/R2H-PDFwrap.py --venv -b all -t
...
def _test_get_m_command():
    ...
    R2H-PDF_root = os.path.abspath( f'{__file__}/../../../')
    infix = 'CXX=c++ ' if state.state_.openbsd else ''

    test(
            'shared-release',
            f'cd {R2H-PDF_root} && {infix}gmake HAVE_GLUT=no HAVE_PTHREAD=yes verbose=yes shared=yes build=release build_prefix=shared-',
            )
    test(
            'R2H-PDFpy-amd64-shared-release',
            f'cd {R2H-PDF_root} && {infix}gmake HAVE_GLUT=no HAVE_PTHREAD=yes verbose=yes shared=yes build=release build_prefix=R2H-PDFpy-amd64-shared-',
            )
    test(
            'R2H-PDFpy-amd64-fpic-release',
            f'cd {R2H-PDF_root} && CFLAGS="-fPIC" {infix}gmake HAVE_GLUT=no HAVE_PTHREAD=yes verbose=yes build=release build_prefix=R2H-PDFpy-amd64-fpic-',
            )
...
def get_so_version( build_dirs):
    '''
    Returns `.<minor>.<patch>` from include/R2H-PDF/fitz/version.h.
    ...
    d = dict()
    def get_v( name):
        path = f'{build_dirs.dir_R2H-PDF}/include/R2H-PDF/fitz/version.h'
        with open( path) as f:
            for line in f:
...
def _get_m_command( build_dirs, j=None, make=None, m_target=None, m_vars=None):
    '''
    Generates a `make` command for building with `build_dirs.dir_R2H-PDF`.
    ...
    #jlib.log( '{build_dirs.dir_R2H-PDF=}')
    if not make:
        make = os.environ.get('R2H-PDF_MAKE')
        if make:
            jlib.log('Overriding from $R2H-PDF_MAKE: {make=}.')
    if not make:
    ...
    if m_target:
        ...
        make_args += f' libs libR2H-PDF-threads'
    command = f'cd {build_dirs.dir_R2H-PDF} &&'
    if make_env:
        command += make_env
...
def _windows_vs_upgrade( vs_upgrade, build_dirs, devenv):
    ...
    {build_dirs.dir_R2H-PDF}/platform/win32-vs-upgrade/ tree with upgraded .sln
    ...
    if infix is None:
        ...
        prefix1 = f'{build_dirs.dir_R2H-PDF}/platform/win32/'
        prefix2 = f'{build_dirs.dir_R2H-PDF}/platform/{infix}/'
...
def build_0(
    ...
    cpp.cpp_source(
            build_dirs.dir_R2H-PDF,
            namespace,
            f'{build_dirs.dir_R2H-PDF}/platform/c++',
            header_git,
            generated,
            check_regress,
            clang_info_verbose,
    ...
    generated.save(f'{build_dirs.dir_R2H-PDF}/platform/c++')
    ...
    for dir_ in (
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/', '.h',
            ):
...
def build( build_dirs, swig_command, args, vs_upgrade, make_command):
    ...
    '''
    cpp_files   = [
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/classes.cpp',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/classes2.cpp',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/exceptions.cpp',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/functions.cpp',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/internal.cpp',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/implementation/extra.cpp',
            ]
    h_files = [
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/R2H-PDF/classes.h',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/R2H-PDF/classes2.h',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/R2H-PDF/exceptions.h',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/R2H-PDF/functions.h',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/R2H-PDF/internal.h',
            f'{build_dirs.dir_R2H-PDF}/platform/c++/include/R2H-PDF/extra.h',
            ]
    build_python = True
    build_csharp = False
    check_regress = False
        ...
        with jlib.LogPrefixScope( f'{action}: '):
            ...
            if action == '.':
                ...
                if state.state_.windows:
                    ...
                    jlib.log( 'Building libR2H-PDF.so ...')
                    ...
                    p = f'{actual_build_dir}/libR2H-PDF{suffix2}{so_version}'
                    ...
                    if actual_build_dir != build_dirs.dir_so:
                        ...
                        # build/shared-release/libR2H-PDF.so, so we need
                        # to copy into build/shared-release-x64-py3.8/.
                        #
                        jlib.fs_copy( f'{actual_build_dir}/libR2H-PDF{suffix2}', f'{build_dirs.dir_so}/libR2H-PDF{suffix2}', verbose=1)
                ...
                # Compile and link generated C++ code to create libR2H-PDFcpp.so.
                if state.state_.windows:
                    # We build R2H-PDFcpp.dll using the .sln; it will
                    # contain all C functions internally - there is
                    # no R2H-PDF.dll.
                    #
                    win32_infix = _windows_vs_upgrade( vs_upgrade, build_dirs, devenv)
                    jlib.log(f'Building R2H-PDFcpp.dll by running devenv ...')
                    build = f'{windows_build_type2}|{build_dirs.cpu.windows_config}'
                    command = (
                            f'cd {build_dirs.dir_R2H-PDF}&&'
                            f'"{devenv}"'
                            f' platform/{win32_infix}/R2H-PDF.sln'
                            f' /Build "{build}"'
                            )
                    projects = ['R2H-PDFcpp', 'libmuthreads']
                    if m_target:
                    ...
                    for suffix in ('.dll', '.pdb'):
                        jlib.fs_copy(
                                f'{build_dirs.dir_R2H-PDF}/platform/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/R2H-PDFcpp{build_dirs.cpu.windows_suffix}{suffix}',
                                f'{build_dirs.dir_so}/',
                                verbose=1,
                                )

                else:
                    jlib.log( 'Compiling generated C++ source code to create libR2H-PDFcpp.so ...')
                    include1 = f'{build_dirs.dir_R2H-PDF}/include'
                    include2 = f'{build_dirs.dir_R2H-PDF}/platform/c++/include'
                    cpp_files_text = ''
                    for i in cpp_files:
                        cpp_files_text += ' ' + os.path.relpath(i)
                    libR2H-PDFcpp = f'{build_dirs.dir_so}/libR2H-PDFcpp.so{so_version}'
                    libR2H-PDF = f'{build_dirs.dir_so}/libR2H-PDF.so{so_version}'
                    if pyodide:
                        ...
                        command = ( textwrap.dedent(
                                f'''
                                {compiler}
                                    -o {os.path.relpath(libR2H-PDFcpp)}
                                    -sSIDE_MODULE
                                    {build_dirs.cpp_flags}
                                    -fPIC -shared
                                    -I {include1}
                        ...
                        if state.state_.linux and so_version:
                            link_soname_arg = f'-Wl,-soname,{os.path.basename(libR2H-PDFcpp)}'
                        command = ( textwrap.dedent(
                                f'''
                                {compiler}
                                    -o {os.path.relpath(libR2H-PDFcpp)}
                        ...
                        if command_was_run:
                            macos_patch( libR2H-PDFcpp, f'{build_dirs.dir_so}/libR2H-PDF.dylib{so_version}')
                        if so_version and state.state_.linux:
                            jlib.system(f'ln -sf libR2H-PDFcpp.so{so_version} {build_dirs.dir_so}/libR2H-PDFcpp.so')
                        ...
                        #
                        libR2H-PDFcpp = f'{build_dirs.dir_so}/libR2H-PDFcpp.a'
                        libR2H-PDF = []#[ f'{build_dirs.dir_so}/libR2H-PDF.a', f'{build_dirs.dir_so}/libR2H-PDF-third.a']
                        ...
                        # Create libR2H-PDFcpp.a containing all .cpp.o files.
                        if 0:
                            libR2H-PDFcpp_a = f'{build_dirs.dir_so}/libR2H-PDFcpp.a'
                            command = f'ar cr {libR2H-PDFcpp_a} {" ".join(ofiles)}'
                            jlib.build(
                                    ofiles,
                                    libR2H-PDFcpp_a,
                                    command,
                        ...
                        # Create libR2H-PDFcpp.so from all .cpp and .c files.
                        libR2H-PDFcpp_so = f'{build_dirs.dir_so}/libR2H-PDFcpp.so'
                        alibs = [
                                f'{build_dirs.dir_so}/libR2H-PDF.a',
                                f'{build_dirs.dir_so}/libR2H-PDF-third.a'
                                ]
                        command = textwrap.dedent( f'''
                                {compiler}
                                    {build_dirs.cpp_flags}
                ...
                #generated = cpp.Generated(f'{build_dirs.dir_R2H-PDF}/platform/c++')
                with open( f'{build_dirs.dir_R2H-PDF}/platform/c++/generated.pickle', 'rb') as f:
                ...
                if build_python:
                    jlib.log( 'Generating R2H-PDF_cppyy.py file.')
                ...
                #
                if build_python:
                    jlib.log( 'Compiling/linking generated Python module source code to create _R2H-PDF.{"pyd" if state.state_.windows else "so"} ...')
                if build_csharp:
                    jlib.log( 'Compiling/linking generated C# source code to create R2H-PDFcsharp.{"dll" if state.state_.windows else "so"} ...')
                ...
                if state.state_.windows:
                    ...
                    libdir = f'{build_dirs.dir_R2H-PDF}/platform/win32/'
                    R2H-PDFcpp_lib = f'{build_dirs.dir_R2H-PDF}/platform/win32/'
                    if build_dirs.cpu.bits == 64:
                        libdir += 'x64/'
                    libdir += 'Debug/' if debug else 'Memento/' if memento else 'Release/'
                    libs = list()
                    libs.append(libdir + ('R2H-PDFcpp64.lib' if build_dirs.cpu.bits == 64 else 'R2H-PDFcpp.lib'))
                    if memento:
                        libs.append(libdir + 'libR2H-PDF.lib')
                    ...
                    if build_python:
                        ...
                        # this point.
                        #
                        path_cpp = build_dirs.R2H-PDFcpp_swig_cpp('python')
                        ...
                        if 1:
                            ...
                            build_so_windows(
                                    build_dirs,
                                    path_cpp = path_cpp,
                                    path_so = f'{build_dirs.dir_so}/_R2H-PDF.pyd',
                                    path_lib = f'{build_dirs.dir_so}/_R2H-PDF.lib',
                                    defines = (
                                        'FZ_DLL_CLIENT',
                                        'SWIG_PYTHON_SILENT_MEMLEAK',
                                    ),
                                    includes = (
                                        f'{build_dirs.dir_R2H-PDF}/include',
                                        f'{build_dirs.dir_R2H-PDF}/platform/c++/include',
                                        wp.include,
                                    ),
                                    libs = libs,
                                    libpaths = wp.libs,
                                    debug = debug,
                                    memento = memento,
                                    export = 'PyInit__R2H-PDF',
                                    vs=windows_vs,
                                    )
                        else:
                            # Use VS devenv.
                            env_extra = {
                                    'R2H-PDF_PYTHON_INCLUDE_PATH': f'{wp.include}',
                                    'R2H-PDF_PYTHON_LIBRARY_PATH': f'{wp.libs}',
                                    }
                            jlib.log('{env_extra=}')
                            ...
                            win32_infix = _windows_vs_upgrade( vs_upgrade, build_dirs, devenv)
                            jlib.log('Building R2H-PDFpyswig project')
                            command = (
                                    f'cd {build_dirs.dir_R2H-PDF}&&'
                                    f'"{devenv}"'
                                    f' platform/{win32_infix}/R2H-PDFpyswig.sln'
                                    f' /Build "{windows_build_type}Python|{build_dirs.cpu.windows_config}"'
                                    f' /Project R2H-PDFpyswig'
                                    )
                            jlib.system(command, verbose=1, out='log', env_extra=env_extra)

                            jlib.fs_copy(
                                    f'{build_dirs.dir_R2H-PDF}/platform/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/R2H-PDFpyswig.dll',
                                    f'{build_dirs.dir_so}/_R2H-PDF.pyd',
                                    verbose=1,
                                    )
                    ...
                    if build_csharp:
                        ...
                        # this point.
                        #
                        path_cpp = build_dirs.R2H-PDFcpp_swig_cpp('csharp')
                        path_cpp = os.path.relpath(path_cpp)    # So we don't expose build machine details in __FILE__.
                        assert os.path.exists(path_cpp), f'SWIG-generated file does not exist: {path_cpp}'
                        ...
                        if 1:
                            ...
                            build_so_windows(
                                    build_dirs,
                                    path_cpp = path_cpp,
                                    path_so = f'{build_dirs.dir_so}/R2H-PDFcsharp.dll',
                                    path_lib = f'{build_dirs.dir_so}/R2H-PDFcsharp.lib',
                                    defines = (
                                        'FZ_DLL_CLIENT',
                                    ),
                                    includes = (
                            ...
                            win32_infix = _windows_vs_upgrade( vs_upgrade, build_dirs, devenv)
                            jlib.log('Building R2H-PDFcsharp project')
                            command = (
                                    f'cd {build_dirs.dir_R2H-PDF}&&'
                                    f'"{devenv}"'
                                    f' platform/{win32_infix}/R2H-PDFcsharpswig.sln'
                                    f' /Build "{windows_build_type}Csharp|{build_dirs.cpu.windows_config}"'
                                    f' /Project R2H-PDFcsharpswig'
                                    )
                            jlib.system(command, verbose=1, out='log')

                            jlib.fs_copy(
                                    f'{build_dirs.dir_R2H-PDF}/platform/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/R2H-PDFcsharpswig.dll',
                                    f'{build_dirs.dir_so}/R2H-PDFcsharp.dll',
                                    verbose=1,
                                    )
                    ...
                    # R2H-PDF:platform/python/_R2H-PDF.so. We could instead
                    # generate R2H-PDF.py and _R2H-PDF.so in the --dir-so
                    # directory?
                    #
                    # [While libR2H-PDFcpp.so requires matching
                    # debug/release build of libR2H-PDF.so, it looks
                    # like _R2H-PDF.so does not require a matching
                    # libR2H-PDFcpp.so and libR2H-PDF.so.]
                    #
                    flags_compile = ''
                    flags_link = ''
                    if build_python:
                        ...
                        if state.state_.macos:
                            ...
                            #       _wrap_ll_fz_warn(_object*, _object*) in R2H-PDFcpp_swig-0a6733.o
                            #       _wrap_fz_warn(_object*, _object*) in R2H-PDFcpp_swig-0a6733.o
                            #       ...
                    ...
                    if 'shared' in dir_so_flags:
                        ...
                        libR2H-PDF        = f'{build_dirs.dir_so}/libR2H-PDF.a'
                        libR2H-PDFthird   = f'{build_dirs.dir_so}/libR2H-PDF-third.a'
                        libR2H-PDFcpp     = f'{build_dirs.dir_so}/libR2H-PDFcpp.a'
                    else:
                        assert 0, f'Leaf must start with "shared-" or "fpic-": build_dirs.dir_so={build_dirs.dir_so}'

                    if build_python:
                        cpp_path = build_dirs.R2H-PDFcpp_swig_cpp('python')
                        out_so = f'{build_dirs.dir_so}/_R2H-PDF.so'
                    elif build_csharp:
                        cpp_path = build_dirs.R2H-PDFcpp_swig_cpp('csharp')
                        out_so = f'{build_dirs.dir_so}/R2H-PDFcsharp.so'  # todo: append {so_version} ?
                    ...
                    # Build R2H-PDF2.so
                    if build_python:
                        cpp2_path = f'{build_dirs.dir_R2H-PDF}/platform/python/R2H-PDFcpp2_swig.cpp'
                        out2_so = f'{build_dirs.dir_so}/_R2H-PDF2.so'
                        if jlib.fs_filesize( cpp2_path):
                            jlib.log( 'Compiling/linking R2H-PDF2')
                            command = ( textwrap.dedent(
                                    f'''
                                    {compiler}
                                        ...
                                        {flags_compile}
                                        {flags_link2}
                                        {link_l_flags( [libR2H-PDF, libR2H-PDFcpp])}
                    ...
                    # 'R2H-PDF::PdfObj *', no destructor found.` when used with
                    # R2H-PDFpy. However it's not definitely known that these
                    # diagnostics are spurious - seems to be to do with two
                    # separate SWIG Python APIs (R2H-PDF and R2H-PDFpy's `extra`
                    # module) using the same underlying C library.
                    #
                    sos = []
                    sos.append( f'{build_dirs.dir_so}/libR2H-PDFcpp.so{so_version}')
                    if os.path.basename( build_dirs.dir_so).startswith( 'shared-'):
                        sos.append( f'{build_dirs.dir_so}/libR2H-PDF.so{so_version}')

                    # Use separate compilation/linking - is faster when only
                    # R2H-PDF.so has changed, and Pyodide requires it anyway.
                    ...
                    if command_was_run:
                        macos_patch( out_so,
                                f'{build_dirs.dir_so}/libR2H-PDF.dylib{so_version}',
                                f'{build_dirs.dir_so}/libR2H-PDFcpp.so{so_version}',
                                )
            else:
...
def python_settings(build_dirs, startdir=None):
    ...
    # test .py programme can load R2H-PDF.py and _R2H-PDF.so.
    ...
    if state.state_.windows:
        ...
        # _R2H-PDF.pyd in same directory as R2H-PDF.py.
        #
        wp = wdev.WindowsPython(build_dirs.cpu, build_dirs.python_version)
        ...
        pass
        # We build _R2H-PDF.so using `-Wl,-rpath='$ORIGIN,-z,origin` (see
...
def make_docs( build_dirs, languages_original):
    ...
    out_dir = f'{build_dirs.dir_R2H-PDF}/docs/generated'

    for language in languages:

        if language == 'c':
            do_doxygen( 'R2H-PDF', f'{out_dir}/c', f'{build_dirs.dir_R2H-PDF}/include')

        elif language == 'c++':
            do_doxygen( 'R2H-PDFcpp', f'{out_dir}/c++', f'{build_dirs.dir_R2H-PDF}/platform/c++/include')
            ...
            input_relpath = os.path.relpath( f'{build_dirs.dir_so}/R2H-PDF.py', f'{out_dir}/python')
            jlib.system(
            ...
            path = f'{out_dir}/python/R2H-PDF.html'
...
def make_docs_index( build_dirs, languages_original):
    ...
    with open( top_index_html, 'w') as f:
        git_id = jlib.git_get_id( build_dirs.dir_R2H-PDF)
        git_id = git_id.split( '\n')[0]
                            ...
                            <div class="outer">
                                <div class="inner">
                                    <ul>
                                        ...
                                        <li><a href="c++/html/index.html">C++</a> (generated by Doxygen).
                                        <li><a href="python/R2H-PDF.html">Python</a> (generated by Pydoc).
                                    </ul>
                                    <small>
                                        <p>Generation:</p>
                                        <ul>
                                            ...
                                            <li>Git: {git_id}
                                            <li>Command: <code>./scripts/R2H-PDFwrap.py --doc {languages_original}</code>
                                        </ul>
                                    </small>
                                </div>
                            </div>
        ...
        with jlib.LogPrefixScope( f'{arg}: '):
            ...
            if arg == '-h' or arg == '--help':
                ...
                if path == '-k':
                    keep_going = True
                    path = args.next()
                include_dir = os.path.relpath( f'{build_dirs.dir_R2H-PDF}/include')
                def paths():
                    if path.endswith( '+'):
                        active = False
                ...
                failed_paths = []
                for path in paths():
                    if path.endswith( '/R2H-PDF/pdf/name-table.h'):
                        # Not a normal header.
                        continue
                    if path.endswith( '.h'):
                ...
                for path in jlib.fs_paths( build_dirs.ref_dir):
                    ...
                    path2 = f'{build_dirs.dir_R2H-PDF}/platform/c++/{tail}'
                ...
                for a, b in (
                        (f'{build_dirs.dir_R2H-PDF}/platform/c++/', f'{build_dirs.dir_R2H-PDF}/platform/c++/'),
                        (f'{build_dirs.dir_R2H-PDF}/platform/python/', f'{build_dirs.dir_R2H-PDF}/platform/python/')
                        ):
                ...
                assert 'R2H-PDFwrap_ref' in build_dirs.ref_dir
                jlib.system(
                ...
                jlib.system(
                        f'rsync -ai {build_dirs.dir_R2H-PDF}/platform/c++/implementation {build_dirs.ref_dir}',
                        out='log',
                        )
                jlib.system(
                        f'rsync -ai {build_dirs.dir_R2H-PDF}/platform/c++/include {build_dirs.ref_dir}',
                        out='log',
                        )
                            ...
                            for do_wheel in 0, 1:
                                with jlib.LogPrefixScope(f'toml={toml} pip_upgrade={pip_upgrade} do_wheel={do_wheel}: '):
                                    ...
                                    try:
                                        ...
                                        #system( './scripts/R2H-PDFwrap_test.py')
                                        system( '. pylocal-multi/bin/activate && python -m R2H-PDF')
                ...
                files = generated.h_files + generated.cpp_files + [
                        f'{build_dirs.dir_so}/R2H-PDF.py',
                        f'{build_dirs.dir_R2H-PDF}/platform/c++/fn_usage.txt',
                        ]
                ...
                # 'R2H-PDF/...' tail of each local path for the remote path.
                #
                for i in range( len( files)):
                    files[i] = files[i].replace( '/R2H-PDF/', '/./R2H-PDF/')
                ...
                if not destination.endswith('/'):
                    destination += '/'
                from_ = f'{build_dirs.dir_R2H-PDF}/docs/generated/'
                if ':' in destination:
                ...
                src = f'{build_dirs.dir_R2H-PDF}/scripts/R2H-PDFwrap_test.cpp'
                exe = f'{build_dirs.dir_R2H-PDF}/scripts/R2H-PDFwrap_test.cpp.exe'
                includes = (
                        f' -I {build_dirs.dir_R2H-PDF}/include'
                        f' -I {build_dirs.dir_R2H-PDF}/platform/c++/include'
                        )
                cpp_flags = build_dirs.cpp_flags
                if state.state_.windows:
                    win32_infix = _windows_vs_upgrade( vs_upgrade, build_dirs, devenv=None)
                    windows_build_type = build_dirs.windows_build_type()
                    lib = f'{build_dirs.dir_R2H-PDF}/platform/{win32_infix}/{build_dirs.cpu.windows_subdir}{windows_build_type}/R2H-PDFcpp{build_dirs.cpu.windows_suffix}.lib'
                    vs = wdev.WindowsVS()
                    command = textwrap.dedent(f'''
                            "{vs.vcvars}"&&"{vs.cl}"
                                /Tp{src}
                    ...
                    dir_so_flags = os.path.basename( build_dirs.dir_so).split( '-')
                    if 'shared' in dir_so_flags:
                        libR2H-PDF        = f'{build_dirs.dir_so}/libR2H-PDF.so'
                        libR2H-PDFthird   = f''
                        libR2H-PDFcpp     = f'{build_dirs.dir_so}/libR2H-PDFcpp.so'
                    elif 'fpic' in dir_so_flags:
                        libR2H-PDF        = f'{build_dirs.dir_so}/libR2H-PDF.a'
                        libR2H-PDFthird   = f'{build_dirs.dir_so}/libR2H-PDF-third.a'
                        libR2H-PDFcpp     = f'{build_dirs.dir_so}/libR2H-PDFcpp.a'
                ...
                script_py = os.path.relpath( f'{build_dirs.dir_R2H-PDF}/scripts/R2H-PDFwrap_gui.py')
                if arg == '--test-python-gui':
                    #env_extra[ 'R2H-PDF_trace'] = '1'
                    #env_extra[ 'R2H-PDF_check_refs'] = '1'
                    #env_extra[ 'R2H-PDF_trace_exceptions'] = '1'
                    command = f'{command_prefix} {script_py} {build_dirs.dir_R2H-PDF}/thirdparty/zlib/zlib.3.pdf'
                    jlib.system( command, env_extra=env_extra, out='log', verbose=1)

                else:
                    jlib.log( 'running scripts/R2H-PDFwrap_test.py ...')
                    script_py = os.path.relpath( f'{build_dirs.dir_R2H-PDF}/scripts/R2H-PDFwrap_test.py')
                    command = f'{command_prefix} {script_py}'
                    with open( f'{build_dirs.dir_R2H-PDF}/platform/python/R2H-PDF_test.py.out.txt', 'w') as f:
                        jlib.system( command, env_extra=env_extra, out='log', verbose=1)
                        # Repeat with pdf_reference17.pdf if it exists.
                        path = os.path.relpath( f'{build_dirs.dir_R2H-PDF}/../pdf_reference17.pdf')
                        if os.path.exists(path):
                            jlib.log('Running R2H-PDFwrap_test.py on {path}')
                            command += f' {path}'
                            jlib.system( command, env_extra=env_extra, out='log', verbose=1)
                    ...
                    zlib_pdf = os.path.relpath(f'{build_dirs.dir_R2H-PDF}/thirdparty/zlib/zlib.3.pdf')
                    for args2 in (
                            f'trace {zlib_pdf}',
                ...
                csc, mono, R2H-PDF_cs = csharp.csharp_settings(build_dirs)
                ...
                jlib.fs_copy(
                        f'{build_dirs.dir_R2H-PDF}/thirdparty/zlib/zlib.3.pdf',
                        testfile
                        )
                # Create test file whose name contains unicode character, which
                # scripts/R2H-PDFwrap_test.cs will attempt to open.
                ...
                if 1:
                    ...
                    out = 'test-csharp.exe'
                    jlib.build(
                            (f'{build_dirs.dir_R2H-PDF}/scripts/R2H-PDFwrap_test.cs', R2H-PDF_cs),
                            out,
                ...
                # required...
                #
                # We need -unsafe for copying bitmap data from R2H-PDF.
                #
                references = '-r:System.Drawing -r:System.Windows.Forms' if state.state_.linux else ''
                out = 'R2H-PDFwrap_gui.cs.exe'
                jlib.build(
                        (f'{build_dirs.dir_R2H-PDF}/scripts/R2H-PDFwrap_gui.cs', R2H-PDF_cs),
                        out,
                ...
                if state.state_.windows:
                    ...
                    # libraries.
                    jlib.fs_copy(f'{build_dirs.dir_R2H-PDF}/thirdparty/zlib/zlib.3.pdf', f'{build_dirs.dir_so}/zlib.3.pdf')
                    # Note that this doesn't work remotely.
                    out_rel = os.path.relpath( out, build_dirs.dir_so)
                    jlib.system(f'cd {build_dirs.dir_so} && {mono} {out_rel}', verbose=1)
                else:
                    jlib.fs_copy(f'{build_dirs.dir_R2H-PDF}/thirdparty/zlib/zlib.3.pdf', f'zlib.3.pdf')
                    jlib.system(f'LD_LIBRARY_PATH={build_dirs.dir_so} {mono} ./{out}', verbose=1)
                ...
                while 1:
                    ...
                    if arg.startswith('-'):
                        ...
                        tests = arg
                        break
                startdir = os.path.abspath('../PyR2H-PDF/tests')
                ...
                if tests == 'all':
                    jlib.system(
                            f'cd ../PyR2H-PDF/tests && py.test-3 {opts}',
                            env_extra=env_extra,
                            out='log',
                            verbose=1,
                            )
                    ...
                    e = 0
                    for script in sorted(glob.glob( '../PyR2H-PDF/tests/test_*.py')):
                        script = os.path.basename(script)
                        ee = jlib.system(
                                f'cd ../PyR2H-PDF/tests && py.test-3 {opts} {script}',
                                env_extra=env_extra,
                                out='log',
                                verbose=1,
                                raise_errors=0,
                        ...
                        if not e:
                            e = ee
                elif not os.path.isfile(f'../PyR2H-PDF/tests/{tests}'):
                    ts = glob.glob("../PyR2H-PDF/tests/*.py")
                    ts = [os.path.basename(t) for t in ts]
                    raise Exception(f'Unrecognised tests={tests}. Should be "all", "iter" or one of {ts}')
                else:
                    jlib.system(
                            f'cd ../PyR2H-PDF/tests && py.test-3 {opts} {tests}',
                            env_extra=env_extra,
                            out='log',
                            verbose=1,
                            )
                ...
                commands = [
                            ...
                            f'python scripts/R2H-PDFwrap_test.py',
                            f'deactivate',
                            ]
                command = 'true'
                for c in commands:
                ...
                venv = f'venv-R2H-PDFwrap-{state.python_version()}-{state.cpu_name()}'
                ...
                R2H-PDF_root = os.path.abspath( f'{__file__}/../../../')
                sys.path.insert(0, f'{R2H-PDF_root}')
                import setup
                del sys.path[0]
