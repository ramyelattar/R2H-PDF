...
def declaration_text(
    ...
    expand_typedef:
        .
    top_level:
        Prefix used for top-level names. When generating C# code, use `R2H-PDF.`.
...
def make_fncall( tu, cursor, return_type, fncall, out, refcheck_if, trace_if):
    ...
    # has a very small effect on R2H-PDFpy test suite performance - e.g. reduce
    ...
    # Output code that writes diagnostics to std::cerr if $R2H-PDF_trace is set.
    #
    def varname_enable():
    ...
    if cursor.spelling == 'fz_warn':
        ...
        if not uses_fz_context:
            out.write( f'    /* No fz_context* arg, so no need for fz_try()/fz_catch() to convert R2H-PDF exceptions into C++ exceptions. */\n')
        out.write(  f'    {indent}')
        if return_type != 'void':
            out.write(  f'ret = ')
        out.write(  f'{fncall};\n')
    ...
    if uses_fz_context:
        ...
        out.write( f'                std::cerr << __FILE__ << ":" << __LINE__ << ":" << __FUNCTION__ << "(): R2H-PDF error stack size changed by {cursor.spelling}(): " << stack0 << " -> " << stack1 << "\\n";\n')
        out.write( f'        }}\n')
        out.write( f'    #endif\n')
...
def make_outparam_helper(
    ...
    # Write struct.
    generated.swig_cpp.write( 'namespace R2H-PDF\n')
    generated.swig_cpp.write('{\n')
    ...
    for arg in parse.get_args( tu, cursor):
        ...
        # swig - swig maps int64_t to R2H-PDF.SWIGTYPE_p_int64_t which can't be
        # treated or converted to an integer.
        #
        # We also value-initialise in case the underlying R2H-PDF function also
...
def make_python_class_method_outparam_override(
    ...
    function or method to make it call the underlying R2H-PDF function's Python
    ...
    to the underlying R2H-PDF function and wrap the out-params into wrapper
    classes.
    '''
    ...
    # return ret, a, b.
    #
    # We convert returned items to wrapper classes if they are R2H-PDF types.
    #
    out.write( '    return ')
    sep = ''
    if cursor.result_type.spelling != 'void':
...
def function_wrapper(
    ...
    Example generated function:
        ...
        fz_band_writer * R2H-PDF_new_band_writer_of_size(fz_context *ctx, size_t size, fz_output *out)
        {
            fz_band_writer * ret;
            fz_try(ctx) {
...
def make_extra( out_extra_h, out_extra_cpp):
    ...
    These are written in C++ but are at the same level of abstraction as R2H-PDF
...
def make_internal_functions( namespace, out_h, out_cpp, refcheck_if, trace_if):
    ...
    cpp_text = textwrap.dedent(
            ...
            {{
                ...
                {{
                    std::cerr << "*** R2H-PDF C++ build/configuration failure.\\n";
            ...
            {{
                std::cerr << file << ":" << line << ":" << fn << "(): "
                        << "R2H-PDF C++ internal assert failure: " << expression
                        << "\\n" << std::flush;
                abort();
            }}
            ...
            {trace_if}
                static const int    s_trace = R2H-PDF::internal_env_flag("R2H-PDF_trace");
            #else
                static const int    s_trace = R2H-PDF::internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace");
            #endif
            ...
            {{
                ...
                {{
                    ...
                    const char* s = getenv( "R2H-PDF_mt_ctx");
                ...
                /* Provide thread support to R2H-PDF. */
            ...
            {{
                ...
                {{
                    ...
                    we have been destructed. This can happen if global R2H-PDF
                    ...
                    {{
                        std::cerr << __FILE__ << ":" << __LINE__ << ":" << __FUNCTION__ << ":\\n"
                                ...
                                << "***\\n"
                                << "*** This can happen if R2H-PDF wrapper class instances are\\n"
...
def make_function_wrappers(
    ...
    generated.swig_python_exceptions.write( textwrap.dedent( f'''
            ...
            /* Called on startup by R2H-PDF.py, with a list of error classes
            ...
            /* Sets Python exception to a new R2H-PDF.<name> object constructed
            with `text`. */
    ...
    generated.swig_csharp_exceptions.write(textwrap.dedent(f'''
            ...
            the different R2H-PDF C++ exception types into the equivalent C#
    ...
    for i, (enum, typename, padding) in enumerate(errors()):
        ...
        generated.swig_python_exceptions.write( textwrap.dedent( f'''
                /**/
                    catch (R2H-PDF::{typename}& e)
                    {{
                        if (g_R2H-PDF_trace_exceptions)
                        {{
                            std::cerr
                                    << __FILE__ << ':' << __LINE__ << ':'
                                    #ifndef _WIN32
                                    << __PRETTY_FUNCTION__ << ':'
                                    #endif
                                    << " Converting C++ std::exception R2H-PDF::{typename} ({i=}) into Python exception:\\n"
                                    << "    e.m_code: " << e.m_code << "\\n"
                                    << "    e.m_text: " << e.m_text << "\\n"
                                    << "    e.what(): " << e.what() << "\\n"
                ...
                {{
                    ...
                    {{
                        std::cerr
                                ...
                                << __PRETTY_FUNCTION__ << ':'
                                #endif
                                << " Converting C++ std::exception R2H-PDF::FzErrorBase ({error_classes_n-1=}) into Python exception:\\n"
                                << "    e.m_code: " << e.m_code << "\\n"
                                << "    e.m_text: " << e.m_text << "\\n"
                                << "    e.what(): " << e.what() << "\\n"
                                << "    typeid(e).name(): " << typeid(e).name() << "\\n"
    ...
    generated.swig_python_set_error_classes.write( textwrap.dedent( f'''
            # This must be after the declaration of R2H-PDF::FzError*
            # classes in R2H-PDF/exceptions.h and declaration of
...
def class_find_constructor_fns( tu, classname, struct_name, base_name, extras):
    ...
    struct_name:
        Name of underlying R2H-PDF struct.
    base_name:
...
def function_wrapper_class_aware_body(
    ...
    struct_cursor:
        .
    fn_cursor:
        Cursor for the underlying R2H-PDF function.
    return_cursor:
    ...
    out_cpp.write( f'        std::cerr << __FILE__ << ":" << __LINE__ << ":" << __FUNCTION__ << "():"\n')
    out_cpp.write( f'                << " calling R2H-PDF::{rename.ll_fn(fnname)}()\\n";\n')
    out_cpp.write( f'    }}\n')
    out_cpp.write( f'    #endif\n')
    ...
    if fn_cursor.type.is_function_variadic():
        ...
        if class_constructor:
            ...
            out_cpp.write( f'    {_make_top_level(return_cursor.spelling)} temp = R2H-PDF::{rename.ll_fn(fnname)}(')
        elif wrap_return == 'pointer':
            out_cpp.write( f'    {_make_top_level(return_cursor.spelling)}* temp = R2H-PDF::{rename.ll_fn(fnname)}(')
        elif wrap_return == 'const pointer':
            out_cpp.write( f'    const {_make_top_level(return_cursor.spelling)}* temp = R2H-PDF::{rename.ll_fn(fnname)}(')
        elif return_void:
            out_cpp.write( f'    R2H-PDF::{rename.ll_fn(fnname)}(')
        else:
            out_cpp.write( f'    auto ret = R2H-PDF::{rename.ll_fn(fnname)}(')
        ...
        if refcounted_return:
            # This R2H-PDF function returns pointer to a struct which uses reference
        ...
        out_cpp.write( f'R2H-PDF::{rename.fn(fnname)}(')
        sep = ''
...
def function_wrapper_class_aware(
    ...
    if class_constructor:
        ...
        if fn_cursor.result_type.kind == state.clang.cindex.TypeKind.POINTER:
            ...
            #
            #   fz_quad_s ret = R2H-PDF_snap_selection(...);
            #   return Quad(&ret);
            #
...
def class_custom_method(
    ...
    Writes custom method as specified by <extramethod>.
        ...
        struct_cursor
            Cursor for definition of R2H-PDF struct.
        classname
    ...
    if 1:   # lgtm [py/constant-conditional-expression]
        ...
        #
        for fnname in re.findall( '(R2H-PDF::[a-zA-Z0-9_]+) *[(]', extramethod.body):
            fnname = util.clip( fnname, 'R2H-PDF::')
...
def pod_class_fns(
    ...
    # Write comparison fns, using comparison of underlying R2H-PDF struct.
    out_h.write( f'\n')
...
def class_wrapper_virtual_fnptrs(
    ...
    #   functions to R2H-PDF's function pointers (which use things like size_t).
    #
...
def refcount_check_code( out, refcheck_if):
    ...
    out.write( textwrap.dedent(
            ...
            R2H-PDF structs are not smaller than the number of wrapper class
            instances. Enable at runtime by setting environmental variable
            R2H-PDF_check_refs to "1". */

            static const bool   s_check_refs = internal_env_flag("R2H-PDF_check_refs");

            /* For each R2H-PDF struct that has an 'int refs' member, we create
            ...
            enums PDF_ENUM_NAME_* are converted to R2H-PDF.PdfObj's contain
...
def cpp_source(
    ...
    Args:
        ...
        refcheck_if:
            ...
            example by setting `R2H-PDF_trace=1` runtime. For example `#if 1` to
    ...
    assert not dir_R2H-PDF.endswith( '/')
    ...
    for name in (
        ...
        out_hs.add( name, f'{base}/include/R2H-PDF/{name}.h')
    ...
    header_text = textwrap.dedent(
            f'''
            /**
            This file was auto-generated by R2H-PDFwrap.py.
            ''')

    if header_git:
        git_id = jlib.get_git_id( dir_R2H-PDF, allow_none=True)
        if git_id:
            git_id = git_id.split('\n', 1)
            header_text += textwrap.dedent(
                    f'''
    ...
    os.makedirs( f'{base}/include/R2H-PDF', exist_ok=True)
    ...
    header = f'{dir_R2H-PDF}/include/R2H-PDF/fitz.h'
    assert os.path.isfile( header), f'header={header}'

    # Get clang to parse R2H-PDF/fitz.h and R2H-PDF/pdf.h and R2H-PDF/extra.h.
    #
    ...
    # parse it.
    #
    temp_h = f'_R2H-PDFwrap_temp.cpp'
    try:
        ...
        # R2H-PDF_WRAP_LIBCLANG so that extra.h can use dummy definition of
        # std::vector.
        #
        args = [
                '-I', f'{dir_R2H-PDF}/include',
                '-I', f'{dir_R2H-PDF}/platform/c++/include',
                '-D', 'R2H-PDF_WRAP_LIBCLANG',
                '-D', 'FZ_FUNCTION=',
                ]
        tu = index.parse(
                temp_h,
    ...
    out_cpps.exceptions.write( textwrap.dedent(
            ...
            {trace_if}
                static const bool   s_trace_exceptions = R2H-PDF::internal_env_flag("R2H-PDF_trace_exceptions");
            #else
                static const bool   s_trace_exceptions_dummy = R2H-PDF::internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace_exceptions");
            #endif
            '''))
    ...
    out_cpps.classes.write(
            ...
            {trace_if}
                static const int    s_trace = R2H-PDF::internal_env_flag("R2H-PDF_trace");
                static const bool   s_trace_keepdrop = R2H-PDF::internal_env_flag("R2H-PDF_trace_keepdrop");
                static const bool   s_trace_director = R2H-PDF::internal_env_flag("R2H-PDF_trace_director");
            #else
                static const int    s_trace = R2H-PDF::internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace");
                static const bool   s_trace_keepdrop = R2H-PDF::internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace_keepdrop");
                static const bool   s_trace_director = R2H-PDF::internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace_director");
            #endif
            '''))
    ...
    out_cpps.functions.write(
            ...
            {trace_if}
                static const int    s_trace = internal_env_flag("R2H-PDF_trace");
                static const bool   s_trace_keepdrop = internal_env_flag("R2H-PDF_trace_keepdrop");
                static const bool   s_trace_exceptions = internal_env_flag("R2H-PDF_trace_exceptions");
                static const bool   s_check_error_stack = internal_env_flag("R2H-PDF_check_error_stack");
            #else
                static const int    s_trace = internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace");
                static const bool   s_trace_keepdrop = internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace_keepdrop");
                static const bool   s_trace_exceptions = internal_env_flag_check_unset("{trace_if}", "R2H-PDF_trace_exceptions");
                static const bool   s_check_error_stack = internal_env_flag_check_unset("{trace_if}", "R2H-PDF_check_error_stack");
            #endif
    ...
    # Create windows_R2H-PDF.def, containing explicit exports for all R2H-PDF
    # global data and functions. We do this instead of explicitly prefixing
    # everything with FZ_FUNCTION or FZ_DATA in the R2H-PDF header files.
    #
    windows_def_path = os.path.relpath(f'{base}/windows_R2H-PDF.def')
    windows_def = ''
    windows_def += 'EXPORTS\n'
    ...
    # Add some internal fns that PyR2H-PDF requires.
    for fnname in (
            'FT_Get_First_Char',
            'FT_Get_Next_Char',
            ):
    ...
    for cursor in parse.get_children(tu.cursor):
        ...
        # For some reason after updating R2H-PDF 2020-04-13, clang-python is
    ...
    out_hs.functions.write( textwrap.dedent( '''
            /** Reinitializes the R2H-PDF context for single-threaded use, which
            is slightly faster when calling code is single threaded.

            This should be called before any other use of R2H-PDF.
            */
            FZ_FUNCTION void reinit_singlethreaded();
