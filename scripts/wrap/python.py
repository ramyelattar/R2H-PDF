...
def cppyy_add_outparams_wrapper(
    ...
    if num_out_params:
        ...
        text = ''
        text += f'# Patch R2H-PDF.m{fn_name} to return out-params directly.\n'
        text += f'R2H-PDF_m{fn_name}_original = cppyy.gbl.R2H-PDF.m{fn_name}\n'
        text += f'def R2H-PDF_m{fn_name}( '
        sep = ''
        for arg in parse.get_args( tu, fn_cursor):
            if arg.out_param:
                pass
        ...
        text += f'    ret = R2H-PDF_m{fn_name}_original( '
        sep = ''
        for arg in parse.get_args( tu, fn_cursor):
            if arg.out_param:
        ...
        for arg in parse.get_args( tu, fn_cursor):
            if arg.out_param:
                ...
                sep = ', '
        text += f'\n'
        text += f'cppyy.gbl.R2H-PDF.m{fn_name} = R2H-PDF_m{fn_name}\n'

        # Look for class method that will use R2H-PDF.m<fn_name>.
        #Generated
        ...
        if struct_name:
            ...
            text += f'cppyy.gbl.R2H-PDF.{class_name}.{method_name}_original = cppyy.gbl.R2H-PDF.{class_name}.{method_name}\n'
            text += f'cppyy.gbl.R2H-PDF.{class_name}.{method_name} = R2H-PDF_m{fn_name}\n'
        else:
            pass
