...
'''
Installation script for R2H-PDF Python bindings, using scripts/pipcl.py.
...
Internal testing only - environmental variables:
    ...
    R2H-PDF_SETUP_BUILD_DIR
        Overrides the default build directory.

    R2H-PDF_SETUP_USE_CLANG_PYTHON
...
def build_dir():
    ...
    # are running under.
    #
    ret = os.environ.get('R2H-PDF_SETUP_BUILD_DIR')
    if ret is None:
...
def R2H-PDF_version():
    ...
    If $R2H-PDF_SETUP_VERSION is set we use it directly, asserting that it starts
    with the version string defined in include/R2H-PDF/fitz/version.h.
    ...
    time to the base version in include/R2H-PDF/fitz/version.h. For example
    '1.18.0.20210330.1800'.
    '''
    return R2H-PDF_version_internal()


def R2H-PDF_version_internal(t_tuple=None):
    '''
    ...
    >>> v = R2H-PDF_version_internal(t0tuple)
    >>> print(v, file=sys.stderr)
    >>> assert v.endswith('.202406060000')
    '''
    with open(f'{root_dir()}/include/R2H-PDF/fitz/version.h') as f:
        text = f.read()
    m = re.search('\n#define FZ_VERSION "([^"]+)"\n', text)
    assert m
    base_version = m.group(1)

    # If R2H-PDF_SETUP_VERSION exists, use it.
    #
    ret = os.environ.get('R2H-PDF_SETUP_VERSION')
    if ret:
        log(f'Using version from $R2H-PDF_SETUP_VERSION: {ret}')
    ...
    #
    # We use the R2H-PDF version with a unique(ish) suffix based on the current
    # date and time, so we can make multiple Python releases without requiring
    # an increment to the R2H-PDF version.
    #
...
def sdist():
    '''
    pipcl callback. We run './scripts/R2H-PDFwrap.py -b 0' to create C++ files
    ...
    #
    use_clang_python = get_flag('R2H-PDF_SETUP_USE_CLANG_PYTHON', True)
    use_swig = get_flag('R2H-PDF_SETUP_USE_SWIG', True)
    b = ''
    if use_clang_python:
        b += '0'
    if use_swig:
    ...
    command += f'{sys.executable} ./scripts/R2H-PDFwrap.py -d {build_dir()} -b "{b}"'
    log(f'Running: {command}')
    subprocess.check_call(command, shell=True)
    paths += [
            'build/shared-release/R2H-PDF.py',
            'git-info',
            ...
            'platform/c++/include/R2H-PDF/classes.h',
            'platform/c++/include/R2H-PDF/classes2.h',
            'platform/c++/include/R2H-PDF/exceptions.h',
            'platform/c++/include/R2H-PDF/functions.h',
            'platform/c++/include/R2H-PDF/internal.h',
            'platform/c++/windows_R2H-PDF.def',
...
def build():
    '''
    pipcl callback. Build R2H-PDF C, C++ and Python libraries and return list of
    created files.
    '''
    ...
    # might be tricky.
    #
    use_clang_python = get_flag('R2H-PDF_SETUP_USE_CLANG_PYTHON', not in_sdist())
    use_swig = get_flag('R2H-PDF_SETUP_USE_SWIG', True)
    ...
    b += '3'        # Build SWIG library _R2H-PDF.so.

    command = '' if root_dir() == os.getcwd() else f'cd {os.path.relpath(root_dir())} && '
    command += (
            f'"{sys.executable}" ./scripts/R2H-PDFwrap.py'
            f' -d {build_dir()}'
            f' -b {b}'
            )

    do_build = os.environ.get('R2H-PDF_SETUP_DO_BUILD')
    if do_build == '0':
        # This is a hack for testing.
        log(f'Not doing build because $R2H-PDF_SETUP_DO_BUILD={do_build}')
    else:
        log(f'build(): Building R2H-PDF C, C++ and Python libraries with: {command}')
        subprocess.check_call(command, shell=True)
    ...
    if windows():
        ...
        names = [
                f'{build_dir()}/R2H-PDFcpp{infix}.dll',   # C and C++.
                f'{build_dir()}/_R2H-PDF.pyd',            # Python internals.
                f'{build_dir()}/R2H-PDF.py',              # Python.
                ]
    elif macos():
        ...
        for leaf in os.listdir(build_dir()):
            log( f'    {leaf}')
        names = [
                f'{build_dir()}/libR2H-PDF.dylib',    # C.
                f'{build_dir()}/libR2H-PDFcpp.so',    # C++.
                f'{build_dir()}/_R2H-PDF.so',         # Python internals.
                f'{build_dir()}/R2H-PDF.py',          # Python.
                ]
    else:
        names = [
                pipcl.get_soname(f'{build_dir()}/libR2H-PDF.so'),     # C.
                pipcl.get_soname(f'{build_dir()}/libR2H-PDFcpp.so'),  # C++.
                f'{build_dir()}/_R2H-PDF.so',                         # Python internals.
                f'{build_dir()}/R2H-PDF.py',                          # Python.
                ]
    paths = []
...
* Python bindings for the R2H-PDF PDF library.
* A python module called ``R2H-PDF``.
* Generated from the R2H-PDF C++ API, which is itself generated from the R2H-PDF C API.
...
* R2H-PDF's ``setjmp``/``longjmp`` exceptions are converted to Python exceptions.
...
* Support for R2H-PDF function pointers with SWIG Director classes, allowing R2H-PDF to call Python callbacks.
* Provides a small number of extensions beyond the basic C API:
  ...
  * Simple 'POD' structs have ``__str__()`` methods, for example ``R2H-PDF.Rect`` is represented like: ``(x0=90.51 y0=160.65 x1=501.39 y1=215.6)``.

Example usage
-------------

Minimal Python code that uses the ``R2H-PDF`` module:

::

    import R2H-PDF
    document = R2H-PDF.Document('foo.pdf')


A simple example Python test script (run by ``scripts/R2H-PDFwrap.py -t``) is:

* ``scripts/R2H-PDFwrap_test.py``
...
https://R2H-PDF.com/r/C-and-Python-APIs
...
R2H-PDF_package = pipcl.Package(
        name = 'R2H-PDF',
        version = R2H-PDF_version(),
        root = root_dir(),
        summary = 'Python bindings for R2H-PDF library.',
        ...
        home_page = 'https://R2H-PDF.com/',
        project_url = [
            ('Documentation, https://R2H-PDF.com/r/C-and-Python-APIs/'),
            ('Source, https://git.ghostscript.com/?p=R2H-PDF.git'),
...
def build_wheel( wheel_directory, config_settings=None, metadata_directory=None):
    return R2H-PDF_package.build_wheel(
            wheel_directory,
            config_settings,
            metadata_directory,
            )
...
def build_sdist( sdist_directory, config_settings=None):
    return R2H-PDF_package.build_sdist(
            sdist_directory,
            config_settings,
            )
...
if __name__ == '__main__':
    R2H-PDF_package.handle_argv(sys.argv)
