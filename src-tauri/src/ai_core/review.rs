...
pub fn run_review(
    ...
    for query in queries {
        let qe = embedding_runtime.and_then(|e| {
            if e.is_server_ready() {
                e.embed_query(query).ok()
            } else {
                None
            }
        });
    ...
    if all_results.is_empty() {
        return Ok(DocumentReviewResult {
            ...
            citations: vec![],
            warnings: vec![
                "No relevant document chunks found. Build the RAG index first.".to_string(),
            ],
    ...
    // 2. Build citations.
    let citations: Vec<Citation> = all_results
        .iter()
        .enumerate()
        .map(|(i, r)| Citation {
    ...
    let context: String = all_results
        ...
        .map(|r| {
            format!(
                "[Page {}] {}",
                r.page_index + 1,
                r.text.chars().take(400).collect::<String>()
            )
        })
        .collect::<Vec<_>>()
        .join("\n\n");
    ...
    let (summary, mut findings, suggested_actions) =
        parse_review_output(&gen_result.text, &citations, &mut warnings);
    ...
    let eng_findings: Vec<ReviewFinding> = engineering_findings
        .iter()
        .map(|ef| ReviewFinding {
            finding_id: ef.finding_id.clone(),
            title: ef.title.clone(),
            description: ef.description.clone(),
            severity: match ef.severity {
                crate::engineering::types::FindingSeverity::Info => "info",
                crate::engineering::types::FindingSeverity::Warning => "warning",
                crate::engineering::types::FindingSeverity::Major => "major",
                crate::engineering::types::FindingSeverity::Critical => "critical",
            }
            .to_string(),
...
) -> (String, Vec<ReviewFinding>, Vec<ReviewSuggestedAction>) {
    let json_str = extract_json(output);
    let Some(json_str) = json_str else {
        warnings
            .push("LLM output did not contain valid JSON. Using raw text as summary.".to_string());
    ...
    if let Some(arr) = parsed.get("suggested_actions").and_then(|v| v.as_array()) {
        for (i, item) in arr.iter().enumerate() {
            ...
            let action_type = item
                ...
                .to_string();
            if !SAFE_ACTION_TYPES.contains(&action_type.as_str()) {
                continue;
            }
...
pub fn build_compare_review_prompt(
    ...
    for (ctype, page_one_based, old, new_) in text_changes.iter().take(40) {
        ...
        let n = new_
            ...
            .collect::<String>();
        prompt.push_str(&format!(
            "- [{ctype}] page {page_one_based} OLD={o:?} NEW={n:?}\n"
        ));
    }
    if text_changes.len() > 40 {
        prompt.push_str(&format!(
            "...and {} more text changes\n",
            text_changes.len() - 40
        ));
    }
    prompt.push_str(&format!(
...
// The parameters mirror the fields of a compare result one-to-one so the
// single call site can pass the result straight through; bundling them into
// a struct would duplicate the shape without adding clarity.
...
pub fn run_compare_review(
    ...
    let default_model = runtime.registry.default_model().ok_or_else(|| {
        "No default local AI model configured. Open Models tab to install one.".to_string()
    })?;
...
mod tests {
    ...
    fn engineering_findings_included() {
        ...
        let eng_review: Vec<ReviewFinding> = eng_findings
            .iter()
            .map(|ef| ReviewFinding {
                finding_id: ef.finding_id.clone(),
                title: ef.title.clone(),
    ...
    fn compare_review_prompt_lists_base_and_revised_docs() {
        let prompt =
            build_compare_review_prompt("base.pdf", "revised.pdf", "+3 / -1 / ~2", &[], 0, &[]);
