...
pub fn parse_units(text: &str) -> UnitParseResult {
    ...
    for cap in re_power_w().captures_iter(text) {
        if let Some(num) = cap.get(1).and_then(|m| m.as_str().parse::<f64>().ok()) {
            ...
            values.push(NormalizedValue {
                original_text: cap[0].to_string(), numeric_value: num,
                original_unit: orig_unit, normalized_value: norm_val,
                normalized_unit: norm_unit, confidence: 0.95,
            });
        }
    }
    ...
    for cap in re_current().captures_iter(text) {
        if let Some(num) = cap.get(1).and_then(|m| m.as_str().parse::<f64>().ok()) {
            values.push(NormalizedValue {
                original_text: cap[0].to_string(), numeric_value: num,
                original_unit: EngineeringUnit::A, normalized_value: num,
                normalized_unit: EngineeringUnit::A, confidence: 0.9,
            });
        }
    }
    ...
    for cap in re_voltage().captures_iter(text) {
        if let Some(num) = cap.get(1).and_then(|m| m.as_str().parse::<f64>().ok()) {
            values.push(NormalizedValue {
                original_text: cap[0].to_string(), numeric_value: num,
                original_unit: EngineeringUnit::V, normalized_value: num,
                normalized_unit: EngineeringUnit::V, confidence: 0.9,
            });
        }
    }
    ...
    for cap in re_power_factor().captures_iter(text) {
        if let Some(num) = cap.get(1).and_then(|m| m.as_str().parse::<f64>().ok()) {
            if num > 0.0 && num <= 1.0 {
                values.push(NormalizedValue {
                    original_text: cap[0].to_string(), numeric_value: num,
                    original_unit: EngineeringUnit::PowerFactor, normalized_value: num,
                    normalized_unit: EngineeringUnit::PowerFactor, confidence: 0.85,
                });
            } else {
...
fn re_power_w() -> Regex { Regex::new(r"(?i)(\d+(?:\.\d+)?)\s*(kw|w)\b").unwrap() }
fn re_power_va() -> Regex { Regex::new(r"(?i)(\d+(?:\.\d+)?)\s*(kva|va)\b").unwrap() }
fn re_current() -> Regex { Regex::new(r"(?i)(\d+(?:\.\d+)?)\s*(?:amps?|a)\b").unwrap() }
fn re_voltage() -> Regex { Regex::new(r"(?i)(\d+(?:\.\d+)?)\s*(?:volts?|v)\b").unwrap() }
fn re_power_factor() -> Regex { Regex::new(r"(?i)(?:pf|power\s*factor|cos\s*[φϕ])\s*[:=]?\s*(\d+(?:\.\d+)?)").unwrap() }

#[cfg(test)]
mod tests {
    use super::*;

    #[test] fn parse_1200_w() {
