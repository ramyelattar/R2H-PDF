...
pub fn extract_load_schedule(
    ...
    let electrical_pattern =
        Regex::new(r"(?i)(kw|kva|va|amp|panel|circuit|load|mdb|smdb|mcc|db\b)").unwrap();

    for (page_index, text, source) in page_texts {
        pages_scanned += 1;
        if !electrical_pattern.is_match(text) {
            continue;
        }
        ...
        for line in text.lines() {
            ...
            if !electrical_pattern.is_match(trimmed) {
                continue;
            }
            ...
            let mut row = LoadScheduleRow {
                ...
                phase: None,
                power_factor: None,
                circuit_ref: extract_circuit_ref(trimmed),
            ...
            // Extract phase.
            if let Some(ph) = extract_phase(trimmed) {
                row.phase = Some(ph);
            }
    ...
    if rows.is_empty() {
        warnings.push(
            "No electrical load schedule was detected in the indexed document text.".to_string(),
        );
    }
    ...
    re.captures(line).and_then(|c| c.get(1)).map(|m| {
        ...
        if s.is_empty() {
            line.split_whitespace()
                .find(|w| Regex::new(r"(?i)^(mdb|smdb|db|mcc)").unwrap().is_match(w))
                .unwrap_or("")
                .to_string()
        } else {
            s
...
fn extract_phase(line: &str) -> Option<String> {
    let re = Regex::new(r"(?i)\b(3[- ]?ph(?:ase)?|single[- ]?ph(?:ase)?|1[- ]?ph(?:ase)?|3P|1P)\b")
        .unwrap();
...
fn generate_extraction_findings(rows: &[LoadScheduleRow]) -> Vec<EngineeringFinding> {
    ...
    let mut circuit_counts: std::collections::HashMap<&str, usize> =
    ...
    for (tag, count) in &tag_counts {
        if *count > 1 {
            findings.push(EngineeringFinding {
                finding_id: format!("dup-tag-{}", tag),
                finding_type: "duplicate_equipment_tag".into(),
                severity: if *count > 3 {
                ...
                recommendation: "Verify equipment tags are unique".into(),
                confidence: 0.9,
            });
        }
    }
    ...
    for (cref, count) in &circuit_counts {
        if *count > 1 {
            findings.push(EngineeringFinding {
                finding_id: format!("dup-cct-{}", cref),
                finding_type: "duplicate_circuit_ref".into(),
                ...
                source_citations: vec![],
                calculation_trace_id: None,
                recommendation: "Verify circuit references are unique".into(),
                confidence: 0.85,
            });
        }
    }
...
mod tests {
    ...
    #[test]
    fn extract_simple_row() {
        let pages = vec![(
            0,
            "AHU-1 Air Handling Unit 15 kW 230V 3-phase".to_string(),
    ...
    #[test]
    fn no_schedule_returns_warning() {
        let pages = vec![(
            0,
            "This is a legal document with no electrical content.".to_string(),
            "native_text".to_string(),
        )];
        let result = extract_load_schedule("s1", &pages);
        assert!(result.rows.is_empty());
        assert!(result
            .warnings
            .iter()
            .any(|w| w.contains("No electrical load schedule")));
    }
    ...
    fn detects_duplicate_tags() {
        ...
        assert!(result
            .findings
            .iter()
            .any(|f| f.finding_type == "duplicate_equipment_tag"));
    }
}
