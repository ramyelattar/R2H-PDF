...
pub fn sum_connected_load(inputs: &[CalculationInput]) -> CalculationTrace {
    let kw_inputs: Vec<&CalculationInput> = inputs
        .iter()
        .filter(|i| matches!(i.unit, EngineeringUnit::KW))
        .collect();

    let total: f64 = kw_inputs.iter().map(|i| i.value).sum();
    let citations: Vec<String> = kw_inputs
        .iter()
        .filter_map(|i| i.source_citation.clone())
        .collect();
    ...
    CalculationTrace {
        ...
        warnings: if kw_inputs.is_empty() {
            vec!["No kW values provided".to_string()]
        } else {
            vec![]
        },
...
pub fn sum_apparent_load(inputs: &[CalculationInput]) -> CalculationTrace {
    ...
    CalculationTrace {
        ...
        warnings: if kva_inputs.is_empty() {
            vec!["No kVA values provided".to_string()]
        } else {
            vec![]
        },
        source_citations: citations,
...
pub fn kw_to_kva(kw: f64, pf: Option<f64>) -> CalculationTrace {
    ...
    let (result, formula) = match pf {
        ...
        _ => {
            warnings.push(
                "Power factor is missing or invalid. Cannot convert kW to kVA without PF."
                    .to_string(),
            );
    ...
    CalculationTrace {
        ...
        steps: vec![CalculationStep {
            step_id: "s1".to_string(),
            description: "kVA = kW / PF".to_string(),
            formula,
...
pub fn kva_to_kw(kva: f64, pf: Option<f64>) -> CalculationTrace {
    ...
    let (result, formula) = match pf {
        ...
        _ => {
            warnings.push(
                "Power factor is missing or invalid. Cannot convert kVA to kW without PF."
                    .to_string(),
            );
    ...
    CalculationTrace {
        ...
        steps: vec![CalculationStep {
            step_id: "s1".into(),
            description: "kW = kVA × PF".into(),
            formula,
            inputs: vec![CalculationInput {
                id: "kva".into(),
                label: "Apparent Power".into(),
                value: kva,
...
pub fn three_phase_kva(voltage: f64, current: f64) -> CalculationTrace {
    ...
    CalculationTrace {
        ...
        steps: vec![CalculationStep {
            step_id: "s1".into(),
            description: "kVA = √3 × V × A / 1000".into(),
...
mod tests {
    ...
    fn inp(id: &str, val: f64, unit: EngineeringUnit) -> CalculationInput {
        CalculationInput {
            ...
            value: val,
            unit,
            source_citation: Some(format!("page-{}", id)),
        }
    }

    #[test]
    fn sum_kw() {
        let t = sum_connected_load(&[
            inp("a", 1.5, EngineeringUnit::KW),
            inp("b", 2.5, EngineeringUnit::KW),
        ]);
