...
impl Default for ContentEditHistoryState {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ContentEditHistoryState {
    ...
    pub fn add_record_with_snapshot(&self, mut record: ContentEditRecord, snapshot: Vec<u8>) {
        if let Ok(mut records) = self.records.lock() {
            ...
            let session_snapshots: usize = records
                .iter()
                .filter(|r| r.session_id == record.session_id && r.before_stream_snapshot.is_some())
                .count();
            if session_snapshots >= MAX_SNAPSHOTS {
                // Remove oldest snapshot.
                if let Some(oldest) = records.iter_mut().find(|r| {
                    r.session_id == record.session_id && r.before_stream_snapshot.is_some()
                }) {
    ...
    pub fn revert_edit(
        ...
        let snapshot = {
            ...
            let record = records
                .iter()
                .find(|r| r.edit_id == edit_id && r.session_id == session_id)
