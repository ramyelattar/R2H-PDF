...
fn default_auto_ocr_scanned() -> bool {
    false
}
...
impl Default for CompareResultStore {
    fn default() -> Self {
        Self::new()
    }
}
...
impl CompareResultStore {
    ...
    pub fn insert(&self, result: CompareResult) -> Result<(), String> {
        let mut g = self
            .inner
            .lock()
            .map_err(|_| "compare store lock poisoned".to_string())?;
...
impl CompareState {
    pub fn new() -> Self {
        Self {
            store: std::sync::Arc::new(CompareResultStore::new()),
        }
    }
}
...
pub fn run_text_compare(
    ...
    let base: Vec<PageTextWithSource> = base_pages
        .iter()
        .map(|t| PageTextWithSource {
            text: t.clone(),
            source: PageTextSource::NativeText,
            lines: None,
        })
        .collect();
    let revised: Vec<PageTextWithSource> = revised_pages
        .iter()
        .map(|t| PageTextWithSource {
            text: t.clone(),
            source: PageTextSource::NativeText,
...
pub fn run_text_compare_with_sources(
    ...
    for page_idx in 0..max_compare {
        ...
        let page_added = diff
            .iter()
            .filter(|d| matches!(d, IdxDiffOp::Add { .. }))
            .count();
        let page_removed = diff
            .iter()
            .filter(|d| matches!(d, IdxDiffOp::Remove { .. }))
            .count();

        let base_bboxes = base_page
            .and_then(|p| p.lines.as_ref())
            .map(|v| v.as_slice());
        let revised_bboxes = revised_page
            .and_then(|p| p.lines.as_ref())
            .map(|v| v.as_slice());

        let (changes_for_page, page_modified) =
            collapse_modifications_indexed(page_idx, &diff, base_bboxes, revised_bboxes);
        ...
        if net_added + net_removed + page_modified > 0 {
...
fn diff_lines_indexed<'a>(a: &[&'a str], b: &[&'a str]) -> Vec<IdxDiffOp<'a>> {
    ...
    while i > 0 && j > 0 {
        if normalize(a[i - 1]) == normalize(b[j - 1]) {
            ...
            ops.push(IdxDiffOp::Remove {
                text: a[i - 1],
                base_idx: i - 1,
            });
            i -= 1;
        } else {
            ops.push(IdxDiffOp::Add {
                text: b[j - 1],
                revised_idx: j - 1,
            });
            j -= 1;
...
fn collapse_modifications_indexed(
    ...
    while i < ops.len() {
        ...
        let pair = match (&ops[i], ops.get(i + 1)) {
            (
                IdxDiffOp::Remove { text: o, base_idx },
                Some(IdxDiffOp::Add {
                    text: n,
                    revised_idx,
                }),
            ...
            (
                ...
                Some(IdxDiffOp::Remove { text: o, base_idx }),
            ) => Some((*o, *base_idx, *n, *revised_idx)),
            _ => None,
        };
        ...
        match &ops[i] {
            ...
            IdxDiffOp::Add {
                ...
                let bbox = revised_lines
                    .and_then(|v| v.get(*revised_idx))
                    .map(|l| l.bbox);
                out.push(CompareTextChange {
                    page_index,
...
code{font-family:Menlo,Consolas,monospace;font-size:12px;}\
    ...
    html.push_str(&format!(
        ...
        esc(&doc_info_label(&r.base_document)),
        r.base_document.page_count,
    ));
    html.push_str(&format!(
        "<tr><td>Revised</td><td>{}</td><td>{}</td></tr>",
        esc(&doc_info_label(&r.revised_document)),
        r.revised_document.page_count,
    ));
    html.push_str("</table>");
    ...
    html.push_str(&format!(
        "<tr><td>Pages compared</td><td>{}</td></tr>",
        r.summary.pages_compared
    ));
    html.push_str(&format!(
        "<tr><td>Pages with changes</td><td>{}</td></tr>",
        r.summary.pages_with_changes
    ));
    html.push_str(&format!(
        "<tr><td class=\"added\">Lines added</td><td>{}</td></tr>",
        r.summary.lines_added
    ));
    html.push_str(&format!(
        "<tr><td class=\"removed\">Lines removed</td><td>{}</td></tr>",
        r.summary.lines_removed
    ));
    html.push_str(&format!(
        "<tr><td class=\"modified\">Lines modified</td><td>{}</td></tr>",
        r.summary.lines_modified
    ));
    html.push_str(&format!(
        "<tr><td class=\"visual_modified\">Visual regions</td><td>{}</td></tr>",
        r.visual_changes.len()
    ));
    html.push_str(&format!(
        "<tr><td>Identical?</td><td>{}</td></tr>",
        r.summary.identical
    ));
    html.push_str("</table>");
    ...
    html.push_str(
        "<table><tr><th>Page</th><th>Source</th><th>+</th><th>−</th><th>~</th><th>Visual</th></tr>",
    );
    ...
    if r.text_changes.is_empty() {
        ...
        html.push_str(
            "<table><tr><th>Page</th><th>Type</th><th>Old</th><th>New</th><th>Citation</th></tr>",
        );
    ...
    if r.visual_changes.is_empty() {
        ...
        html.push_str(
            "<table><tr><th>Page</th><th>Type</th><th>BBox (pts)</th><th>Confidence</th></tr>",
        );
...
pub fn combine_sources(a: Option<&PageTextSource>, b: Option<&PageTextSource>) -> String {
    use PageTextSource as P;
    let a_val = a.cloned().unwrap_or(P::None);
    let b_val = b.cloned().unwrap_or(P::None);
...
pub fn run_visual_page_diff(
    ...
    if base.width_px == 0
        ...
        || revised.width_px == 0
        || revised.height_px == 0
        || base.pixels_rgba.is_empty()
        || revised.pixels_rgba.is_empty()
    {
        return (Vec::new(), 0);
    }
    ...
    let cells = cell_grid.clamp(4, 128) as usize;
    ...
    for (cx, col) in grid.iter_mut().enumerate() {
        for (cy, cell) in col.iter_mut().enumerate() {
            let x0 = cx * cell_w;
            let y0 = cy * cell_h;
            ...
            for y in y0..y1 {
                ...
                for x in x0..x1 {
                    ...
                    let dr = (base.pixels_rgba[i_a] as i32 - revised.pixels_rgba[i_b] as i32)
                        .unsigned_abs() as u64;
                    let dg = (base.pixels_rgba[i_a + 1] as i32
                        - revised.pixels_rgba[i_b + 1] as i32)
                        .unsigned_abs() as u64;
                    let db = (base.pixels_rgba[i_a + 2] as i32
                        - revised.pixels_rgba[i_b + 2] as i32)
                        .unsigned_abs() as u64;
                    diff_sum += dr + dg + db;
                    pixel_count += 1;
                }
            }
            if let Some(avg_per_pixel) = diff_sum.checked_div(pixel_count).map(|v| v as u32) {
    ...
    for (rx0, ry0, rx1, ry1, count) in &regions {
        ...
        let confidence = ((*count as f32) / 8.0).clamp(0.1, 1.0);
...
mod tests {
    ...
    fn detects_added_line() {
        ...
        assert!(r
            ...
            .any(|c| matches!(c.change_type, ChangeType::Added)
                && c.new_text.as_deref() == Some("gamma")));
    }
    ...
    fn detects_modified_line_via_similarity() {
        ...
        assert_eq!(
            r.summary.lines_modified, 1,
            "expected modified line, got {:?}",
            r
        );
        assert!(r
            .text_changes
    ...
    fn no_unconditional_visual_warning_when_text_only_runs_in_visual_mode() {
        ...
        let r = run_text_compare(
            ...
            info("a", 1),
            info("b", 1),
            CompareMode::PageVisual,
        );
        assert!(
            !r.warnings
                .iter()
                .any(|w| w.to_lowercase().contains("visual diff was skipped")),
            "expected text engine to be quiet about visual mode now, got {:?}",
            r.warnings,
        );
    }
    ...
    fn ocr_text_source_is_tracked_per_page() {
        ...
        let rev = vec![PageTextWithSource {
            text: "hello universe".into(),
            source: PageTextSource::OcrText,
            lines: None,
        }];
    ...
    fn no_text_on_either_side_emits_warning() {
        ...
        assert!(
            r.warnings.iter().any(|w| w.contains("scanned")),
            "expected scanned-page warning, got {:?}",
            r.warnings
        );
    }
    ...
    fn combine_sources_handles_all_combinations() {
        ...
        assert_eq!(
            combine_sources(Some(&P::NativeText), Some(&P::OcrText)),
            "mixed"
        );
        assert_eq!(
            combine_sources(Some(&P::OcrText), Some(&P::OcrText)),
            "ocr_text"
        );
        assert_eq!(
            combine_sources(Some(&P::Mixed), Some(&P::NativeText)),
            "mixed"
        );
    }
    ...
    fn entirely_different_rendered_pages_produce_at_least_one_region() {
        ...
        assert!(changes
            .iter()
            .all(|c| matches!(c.change_type, ChangeType::VisualModified)));
    }
    ...
    fn local_visual_change_returns_localized_bbox() {
        ...
        assert!(
            bbox[3] > page_h * 0.5,
            "expected change in upper half of PDF, got {:?}",
            bbox
        );
    }
    ...
    fn extra_pages_on_revised_count_as_added() {
        ...
        assert!(
            r.summary.lines_added >= 3,
            "expected ≥3 added lines, got {:?}",
            r.summary
        );
    }
    ...
    fn synth_result_for_report(
        text_changes: Vec<CompareTextChange>,
        visual: Vec<CompareVisualChange>,
    ) -> CompareResult {
        CompareResult {
            ...
            page_results: vec![ComparePageResult {
                page_index: 0,
                base_line_count: 2,
                revised_line_count: 2,
                added: 1,
                removed: 1,
                modified: 0,
                text_source: "native_text".into(),
                visual_regions: visual.len(),
            }],
            text_changes,
        ...
        let r = synth_result_for_report(
            vec![
                CompareTextChange {
                    ...
                    change_type: ChangeType::Added,
                    old_text: None,
                    new_text: Some("plus".into()),
                    bbox: None,
                    confidence: 1.0,
                    citation: "page 1".into(),
                },
                CompareTextChange {
                    page_index: 0,
                    change_type: ChangeType::Removed,
                    old_text: Some("minus".into()),
                    new_text: None,
                    bbox: None,
                    confidence: 1.0,
                    citation: "page 1".into(),
    ...
    fn redline_html_report_escapes_change_text() {
        let r = synth_result_for_report(
            vec![CompareTextChange {
                ...
                change_type: ChangeType::Added,
                old_text: None,
                new_text: Some("<script>alert(1)</script>".into()),
                bbox: None,
                confidence: 1.0,
                citation: "page 1".into(),
            }],
    ...
    fn removed_line_carries_base_side_bbox() {
        ...
        let removed = r
            .text_changes
            .iter()
            .find(|c| matches!(c.change_type, ChangeType::Removed))
            .unwrap();
        assert_eq!(removed.old_text.as_deref(), Some("beta"));
        assert_eq!(
            removed.bbox,
            Some([10.0, 685.0, 50.0, 697.0]),
    ...
    fn added_line_carries_revised_side_bbox() {
        ...
        let added = r
            .text_changes
            .iter()
            .find(|c| matches!(c.change_type, ChangeType::Added))
            .unwrap();
    ...
    fn modified_line_uses_base_side_bbox() {
        ...
        let modified = r
            .text_changes
            .iter()
            .find(|c| matches!(c.change_type, ChangeType::Modified))
            .unwrap();
