...
pub fn build_report_bundle(
    ...
    let (summary, findings, risks, missing_info) = if let Some(rev) = review_result {
        ...
        let missing = rev
            ...
            .map(|f| ReportFinding {
                id: f.finding_id.clone(),
                severity: f.severity.clone(),
                category: "missing_information".into(),
                title: f.title.clone(),
                description: f.description.clone(),
                recommendation: f.recommendation.clone(),
                page_refs: f.page_refs.clone(),
        ...
        for c in &rev.citations {
            all_citations.push(ReportCitation {
    ...
    // Traces.
    let traces: Vec<ReportTrace> = eng_traces
        .iter()
        .map(|t| {
                ...
                .flat_map(|s| {
                    s.inputs.iter().map(|i| {
                        format!(
                            ...
                            i.value,
                            i.unit,
                            i.source_citation.as_deref().unwrap_or("")
                        )
                    })
                })
                .collect();
            let formula = t
                ...
                .unwrap_or_default();
            ReportTrace {
                id: t.calculation_id.clone(),
                title: t.title.clone(),
                calculation_type: t.calculation_type.clone(),
                formula,
                inputs,
                final_value: format!("{:.2} {:?}", t.final_value, t.final_unit),
    ...
    let now = SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let generated_at = chrono_lite(now).to_string();
...
pub fn report_export_review(
    ...
    let eng_findings = eng_state
        .findings
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .clone();
    let eng_traces = eng_state
        .traces
        .lock()
        .unwrap_or_else(|e| e.into_inner())
        .clone();
    ...
    if request.include_review {
        sections.push("review".into());
    }
    if request.include_engineering {
        sections.push("engineering".into());
    }
    if request.include_calculation_traces {
        sections.push("traces".into());
    }
    if request.include_ai_actions {
        sections.push("ai_actions".into());
    }
    if request.include_audit_summary {
        sections.push("audit".into());
    }
    if request.include_appendix {
        sections.push("appendix".into());
    }
...
mod tests {
    ...
    fn build_bundle_from_engineering_only() {
        ...
        let eng = vec![crate::engineering::types::EngineeringFinding {
            finding_id: "ef1".into(),
