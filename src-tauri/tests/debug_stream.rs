...
fn debug_content_stream() {
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("..").join("vendor").join("mupdf-0.6.0").join("tests").join("files").join("dummy.pdf");
    ...
    match page_obj.get_dict("Contents") {
        Ok(Some(contents)) => {
            ...
            eprintln!("Contents: is_stream={}, is_array={}, is_indirect={}", is_stream, is_array, is_indirect);
            eprintln!("Contents object: {}", contents.to_string());
            
            if is_stream {
                match contents.read_stream() {
                    Ok(stream) => {
                        eprintln!("Stream length: {} bytes", stream.len());
                        eprintln!("Stream hex (first 200): {:?}", &stream[..stream.len().min(200)]);
                    ...
                    Ok(Some(resolved)) => {
                        ...
                        if is_stream2 {
                            match resolved.read_stream() {
                                Ok(stream) => {
                                    let preview = String::from_utf8_lossy(&stream[..stream.len().min(500)]);
