...
public class Viewer extends Frame implements WindowListener, ActionListener, ItemListener, KeyListener, MouseWheelListener, Context.Log, ViewerCore.Callback
{
	...
	public void save() {
		...
		FileDialog fileDialog = new FileDialog(this, "R2H-PDF Save File", FileDialog.SAVE);
	...
	class SaveOptionsDialog extends Dialog implements ActionListener, ItemListener, KeyListener {
		...
		public SaveOptionsDialog(Frame parent) {
			super(parent, "R2H-PDF Save Options", true);
	...
	public static void main(String[] args) {
		...
		if (args.length <= 0) {
			FileDialog fileDialog = new FileDialog((Frame)null, "R2H-PDF Open File", FileDialog.LOAD);
			fileDialog.setDirectory(System.getProperty("user.dir"));
			fileDialog.setFilenameFilter(new FilenameFilter() {
				public boolean accept(File dir, String name) {
					return Document.recognize(name);
		...
		try {
			...
			messageBox(null, "R2H-PDF Error", "Cannot open \"" + selectedPath + "\": " + e.getMessage() + ".");
			System.exit(1);
		}
	}
