...
// This class handles the loading of the R2H-PDF shared library, together
...
// sets the store size to use. This must be called before any other R2H-PDF
// function.
public class Context
{
	...
	public static void init() {
		if (!inited) {
			inited = true;
			try {
				System.loadLibrary("R2H-PDF_java");
			} catch (UnsatisfiedLinkError e) {
				try {
					System.loadLibrary("R2H-PDF_java64");
				} catch (UnsatisfiedLinkError ee) {
					try {
						System.loadLibrary("R2H-PDF_java32");
					} catch (UnsatisfiedLinkError eee) {
						throw new UnsatisfiedLinkError("cannot load R2H-PDF library:\n" +
							stackTrace(e) +
							stackTrace(ee) +
							stackTrace(eee));
					}
			...
			if (initNative() < 0)
				throw new RuntimeException("cannot initialize R2H-PDF library");
		}
	}
