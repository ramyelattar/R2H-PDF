...
{
	fz_font *font = EM_ASM_PTR({ return globalThis.$libR2H-PDF_load_font_file($0, $1, $2, $3) }, name, script, bold, italic);
	if (font)
...
void wasm_init_context(void)
{
	...
	if (!ctx)
		EM_ASM({ throw new Error("Cannot create R2H-PDF context!"); });
...
static void js_stm_drop(fz_context *ctx, void *state_)
{
	...
	EM_ASM({
		globalThis.$libR2H-PDF_stm_close($0);
	}, state->id);
	fz_free(ctx, state);
}
...
static int js_stm_next(fz_context *ctx, fz_stream *stm, size_t len)
{
	...
	int n = EM_ASM_INT(
		{
			return globalThis.$libR2H-PDF_stm_read($0, $1, $2, $3);
		},
		(int) state->id,
		(int) stm->pos,
		(int) state->buf,
...
static void js_stm_seek(fz_context *ctx, fz_stream *stm, int64_t offset, int whence)
{
	...
	int n = EM_ASM_INT(
		{
			return globalThis.$libR2H-PDF_stm_seek($0, $1, $2, $3);
		},
		(int) state->id,
		(int) stm->pos,
		(int) offset,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_path_walk.moveto($0, $1, $2) }, (int)arg, x, y);
}

static void
path_walk_lineto(fz_context *ctx, void *arg, float x, float y)
{
	EM_ASM({ globalThis.$libR2H-PDF_path_walk.lineto($0, $1, $2) }, (int)arg, x, y);
}

static void
path_walk_curveto(fz_context *ctx, void *arg, float x1, float y1, float x2, float y2, float x3, float y3)
{
	EM_ASM({ globalThis.$libR2H-PDF_path_walk.curveto($0, $1, $2, $3, $4, $5, $6) }, (int)arg, x1, y1, x2, y2, x3, y3);
}

static void
path_walk_closepath(fz_context *ctx, void *arg)
{
	EM_ASM({ globalThis.$libR2H-PDF_path_walk.closepath($0) }, (int)arg);
}
...
void wasm_walk_text(fz_text *text, int walk_id)
{
	...
	for (span = text->head; span; span = span->next) {
		trm = span->trm;
		EM_ASM({ globalThis.$libR2H-PDF_text_walk.begin_span($0, $1, $2, $3, $4, $5, $6) },
			walk_id,
			span->font,
			&trm,
			span->wmode,
		...
		for (i = 0; i < span->len; ++i) {
			...
			EM_ASM({ globalThis.$libR2H-PDF_text_walk.show_glyph($0, $1, $2, $3, $4, $5, $6, $7) },
				walk_id,
				span->font,
				&trm,
				span->items[i].gid,
		...
		EM_ASM({ globalThis.$libR2H-PDF_text_walk.end_span($0) }, walk_id);
	}
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_log_error($0) }, message);
}

static void
log_warning_callback(void *user, const char *message)
{
	EM_ASM({ globalThis.$libR2H-PDF_log_warning($0) }, message);
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.drop_device($0) }, ((js_device*)dev)->id);
}

static void
js_dev_close_device(fz_context *ctx, fz_device *dev)
{
	EM_ASM({ globalThis.$libR2H-PDF_device.close_device($0) }, ((js_device*)dev)->id);
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.fill_path($0, $1, $2, $3, $4, $5, $6, $7) },
		((js_device*)dev)->id,
		path,
		even_odd,
		&ctm,
...
js_dev_clip_path(fz_context *ctx, fz_device *dev, const fz_path *path, int even_odd, fz_matrix ctm,
	fz_rect scissor)
{
	EM_ASM({ globalThis.$libR2H-PDF_device.clip_path($0, $1, $2, $3) },
		((js_device*)dev)->id,
		path,
		even_odd,
		&ctm
...
js_dev_stroke_path(fz_context *ctx, fz_device *dev, const fz_path *path,
	...
	fz_colorspace *colorspace, const float *color, float alpha, fz_color_params color_params)
{
	EM_ASM({ globalThis.$libR2H-PDF_device.stroke_path($0, $1, $2, $3, $4, $5, $6, $7) },
		((js_device*)dev)->id,
		path,
		stroke,
		&ctm,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.clip_stroke_path($0, $1, $2, $3) },
		((js_device*)dev)->id,
		path,
		stroke,
		&ctm
...
js_dev_fill_text(fz_context *ctx, fz_device *dev, const fz_text *text, fz_matrix ctm,
	fz_colorspace *colorspace, const float *color, float alpha, fz_color_params color_params)
{
	EM_ASM({ globalThis.$libR2H-PDF_device.fill_text($0, $1, $2, $3, $4, $5, $6) },
		((js_device*)dev)->id,
		text,
		&ctm,
		colorspace,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.stroke_text($0, $1, $2, $3, $4, $5, $6, $7) },
		((js_device*)dev)->id,
		text,
		stroke,
		&ctm,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.clip_text($0, $1, $2) },
		((js_device*)dev)->id,
		text,
		&ctm
	);
...
js_dev_clip_stroke_text(fz_context *ctx, fz_device *dev, const fz_text *text, const fz_stroke_state *stroke,
	fz_matrix ctm, fz_rect scissor)
{
	EM_ASM({ globalThis.$libR2H-PDF_device.clip_stroke_text($0, $1, $2, $3) },
		((js_device*)dev)->id,
		text,
		stroke,
		&ctm
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.ignore_text($0, $1, $2) },
		((js_device*)dev)->id,
		text,
		&ctm
	);
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.fill_shade($0, $1, $2, $3) },
		((js_device*)dev)->id,
		shade,
		&ctm,
		alpha
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.fill_image($0, $1, $2, $3) },
		((js_device*)dev)->id,
		image,
		&ctm,
		alpha
...
js_dev_fill_image_mask(fz_context *ctx, fz_device *dev, fz_image *image, fz_matrix ctm,
	fz_colorspace *colorspace, const float *color, float alpha, fz_color_params color_params)
{
	EM_ASM({ globalThis.$libR2H-PDF_device.fill_image_mask($0, $1, $2, $3, $4, $5, $6) },
		((js_device*)dev)->id,
		image,
		&ctm,
		colorspace,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.clip_image_mask($0, $1, $2) },
		((js_device*)dev)->id,
		image,
		&ctm
	);
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.pop_clip($0) },
		((js_device*)dev)->id
	);
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.begin_mask($0, $1, $2, $3, $4, $5) },
		((js_device*)dev)->id,
		&bbox,
		luminosity,
		colorspace,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.end_mask($0, $1) },
		((js_device*)dev)->id,
		tr
	);
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.begin_group($0, $1, $2, $3, $4, $5, $6) },
		((js_device*)dev)->id,
		&bbox,
		colorspace,
		isolated,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.end_group($0) },
		((js_device*)dev)->id
	);
}
...
{
	return EM_ASM_INT({ return globalThis.$libR2H-PDF_device.begin_tile($0, $1, $2, $3, $4, $5, $6) },
		((js_device*)dev)->id,
		&area,
		&view,
		xstep,
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.end_tile($0) },
		((js_device*)dev)->id
	);
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.begin_layer($0, $1) },
		((js_device*)dev)->id,
		name
	);
}
...
{
	EM_ASM({ globalThis.$libR2H-PDF_device.end_layer($0) },
		((js_device*)dev)->id
	);
}
