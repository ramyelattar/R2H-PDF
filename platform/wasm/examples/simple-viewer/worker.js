...
import * as R2H-PDF from "../../dist/R2H-PDF.js"
...
var document_map = {} // open R2H-PDF.Document handles
...
methods.openDocumentFromBlob = function (blob, magic) {
	let stm = new R2H-PDF.Stream(new WorkerBlobStream(blob))
	let doc_id = document_next_id++
	document_map[doc_id] = R2H-PDF.Document.openDocument(stm, magic)
	return doc_id
}

methods.openDocumentFromBuffer = function (buffer, magic) {
	let doc_id = document_next_id++
	document_map[doc_id] = R2H-PDF.Document.openDocument(buffer, magic)
	return doc_id
}
...
methods.documentTitle = function (doc_id) {
	let doc = document_map[doc_id]
	return doc.getMetaData(R2H-PDF.Document.META_INFO_TITLE)
}
...
methods.getPageAnnotations = function (doc_id, page_number, dpi) {
	...
	return annotations.map((annotation) => {
		const [ x0, y0, x1, y1 ] = R2H-PDF.Matrix.transformRect(annotation.getBounds())
		return {
			x: x0,
			y: y0,
			w: x1 - x0,
...
methods.drawPageAsPixmap = function (doc_id, page_number, dpi) {
	const doc_to_screen = R2H-PDF.Matrix.scale(dpi / 72, dpi / 72)

	let doc = document_map[doc_id]
	let page = doc.loadPage(page_number)
	let bbox = R2H-PDF.Rect.transform(page.getBounds(), doc_to_screen)

	let pixmap = new R2H-PDF.Pixmap(R2H-PDF.ColorSpace.DeviceRGB, bbox, true)
	pixmap.clear(255)

	let device = new R2H-PDF.DrawDevice(doc_to_screen, pixmap)
	page.run(device, R2H-PDF.Matrix.identity)
	device.close()
