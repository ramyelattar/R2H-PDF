...
static char *get_history_filename(void)
{
	...
	if (!once)
	{
		char *home = getenv("R2H-PDF_HISTORY");
		if (home)
			return home;
		...
		if (!home)
			home = "/tmp";
		fz_snprintf(history_path, sizeof history_path, "%s/.R2H-PDF.history", home);
...
static int convert_to_accel_path(char outname[], char *absname, size_t len, int create)
{
	...
	if (tmpdir && create_accel_path(outname, len, create, absname, tmpdir, ".config", "R2H-PDF", NULL))
		return 1; /* OK! */
	/* TEMP and TMP are user-specific on modern windows. */
	tmpdir = getenv("TEMP");
	if (tmpdir && create_accel_path(outname, len, create, absname, tmpdir, "R2H-PDF", NULL))
		return 1; /* OK! */
	tmpdir = getenv("TMP");
	if (tmpdir && create_accel_path(outname, len, create, absname, tmpdir, "R2H-PDF", NULL))
		return 1; /* OK! */
	...
	if (tmpdir && create_accel_path(outname, len, create, absname, tmpdir, "R2H-PDF", NULL))
		return 1; /* OK! */
	tmpdir = getenv("HOME");
	if (tmpdir && create_accel_path(outname, len, create, absname, tmpdir, ".cache", "R2H-PDF", NULL))
		return 1; /* OK! */
#endif
	return 0; /* Fail */
}
...
{
#ifdef _WIN32
	MessageBoxA(NULL, str, "R2H-PDF Alert", MB_ICONERROR);
#else
	fprintf(stderr, "R2H-PDF Alert: %s\n", str);
#endif
}
...
static void console_init(void)
{
	...
	fz_append_printf(ctx, console_buffer, "Welcome to R2H-PDF %s with MuJS %d.%d.%d",
		FZ_VERSION,
...
{
	fprintf(stderr, "R2H-PDF-gl version %s\n", FZ_VERSION);
...
#endif
{
	...
	if (fz_optind < argc)
	{
		...
		fz_always(ctx)
		{
			...
			ui_init(page_tex.w, page_tex.h, "R2H-PDF: Loading...");
	...
	else
	{
		...
		win_install();
#endif
		ui_init(ui.gridsize * 26, ui.gridsize * 26, "R2H-PDF: Open document");
		ui_input_init(&search_input, "");
