...
/*
 * Create registry keys to associate R2H-PDF with PDF and XPS files.
 */
...
{
	char buf[512];
	HKEY software, classes, R2H-PDF, dotpdf, dotxps, dotepub, dotfb2;
	...
	OPEN_KEY(classes, "R2H-PDF", R2H-PDF);
	OPEN_KEY(R2H-PDF, "SupportedTypes", supported_types);
	OPEN_KEY(R2H-PDF, "shell", shell);
	OPEN_KEY(shell, "open", open);
	OPEN_KEY(open, "command", command);

	sprintf(buf, "\"%s\" \"%%1\"", argv0);

	SET_KEY(open, "FriendlyAppName", "R2H-PDF");
	SET_KEY(command, "", buf);
	SET_KEY(supported_types, ".pdf", "");
	SET_KEY(supported_types, ".xps", "");
	SET_KEY(supported_types, ".epub", "");
	SET_KEY(pdf_progids, "R2H-PDF", "");
	SET_KEY(xps_progids, "R2H-PDF", "");
	SET_KEY(epub_progids, "R2H-PDF", "");
	SET_KEY(fb2_progids, "R2H-PDF", "");
...
{
	MessageBoxA(hwndframe, msg, "R2H-PDF: Warning", MB_ICONWARNING);
}

void winerror(pdfapp_t *app, char *msg)
{
	MessageBoxA(hwndframe, msg, "R2H-PDF: Error", MB_ICONERROR);
	exit(1);
}
...
{
	MessageBoxA(hwndframe, "The R2H-PDF library supports printing, but this application currently does not", "Print document", MB_ICONWARNING);
}

int winsavequery(pdfapp_t *app)
{
	switch(MessageBoxA(hwndframe, "File has unsaved changes. Do you want to save", "R2H-PDF", MB_YESNOCANCEL))
	{
...
{
	switch(MessageBoxA(hwndframe, query, "R2H-PDF", MB_YESNOCANCEL))
	{
...
static int winfilename(wchar_t *buf, int len)
{
	...
	ofn.lpstrTitle = L"R2H-PDF: Open PDF file";
...
int wingetcertpath(pdfapp_t *app, char *buf, int len)
{
	...
	ofn.lpstrInitialDir = NULL;
	ofn.lpstrTitle = L"R2H-PDF: Select certificate file";
...
int wingetsavepath(pdfapp_t *app, char *buf, int len)
{
	...
	ofn.nMaxFile = PATH_MAX;
	ofn.lpstrInitialDir = NULL;
	ofn.lpstrTitle = L"R2H-PDF: Save PDF file";
...
static void winopen()
{
	...
	SetWindowTextW(hwndframe, L"R2H-PDF");

	menu = GetSystemMenu(hwndframe, 0);
	AppendMenuW(menu, MF_SEPARATOR, 0, NULL);
	AppendMenuW(menu, MF_STRING, ID_ABOUT, L"About R2H-PDF...");
...
{
	const char *msg =
		"usage: R2H-PDF [options] file.pdf [page]\n"
		"\t-p -\tpassword\n"
		"\t-r -\tresolution\n"
	...
	MessageBoxA(NULL, msg, "R2H-PDF: Usage", MB_OK);
	exit(1);
}
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	if (!ctx)
	{
		MessageBoxA(NULL, "Cannot initialize R2H-PDF context.", "R2H-PDF: Error", MB_OK);
		exit(1);
	}
