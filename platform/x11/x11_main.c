...
#define R2H-PDF_icon_bitmap_16_width 16
#define R2H-PDF_icon_bitmap_16_height 16
static unsigned char R2H-PDF_icon_bitmap_16_bits[] = {
...
#define R2H-PDF_icon_bitmap_16_mask_width 16
#define R2H-PDF_icon_bitmap_16_mask_height 16
static unsigned char R2H-PDF_icon_bitmap_16_mask_bits[] = {
...
{
	fprintf(stderr, "R2H-PDF: error: %s\n", msg);
	cleanup(app);
	exit(1);
}
...
{
	fprintf(stderr, "The R2H-PDF library supports printing, but this application currently does not\n");
}
...
static void winopen(void)
{
	...
	if (wmhints)
	{
		...
		xicon = XCreateBitmapFromData(xdpy, xwin,
			(char*)R2H-PDF_icon_bitmap_16_bits,
			R2H-PDF_icon_bitmap_16_width,
			R2H-PDF_icon_bitmap_16_height);
		xmask = XCreateBitmapFromData(xdpy, xwin,
			(char*)R2H-PDF_icon_bitmap_16_mask_bits,
			R2H-PDF_icon_bitmap_16_mask_width,
			R2H-PDF_icon_bitmap_16_mask_height);
		if (xicon && xmask)
		{
	...
	if (classhint)
	{
		classhint->res_name = "R2H-PDF";
		classhint->res_class = "R2H-PDF";
...
{
	fprintf(stderr, "R2H-PDF: discarded changes to document\n");
	/* FIXME: temporary dummy implementation */
	return DISCARD;
}
...
{
	fprintf(stderr, "R2H-PDF-x11 version %s\n", FZ_VERSION);
	exit(1);
}
