...
VERSION_MAJOR = $(shell grep "define FZ_VERSION_MAJOR" include/R2H-PDF/fitz/version.h | cut -d ' ' -f 3)
VERSION_MINOR = $(shell grep "define FZ_VERSION_MINOR" include/R2H-PDF/fitz/version.h | cut -d ' ' -f 3)
VERSION_PATCH = $(shell grep "define FZ_VERSION_PATCH" include/R2H-PDF/fitz/version.h | cut -d ' ' -f 3)
...
ifeq ($(shared),yes)
  ...
  ifeq ($(OS),OpenBSD)
    # OpenBSD linker magic doesn't use soname; so fake it by using -L$(OUT) and -lR2H-PDF.
    LINK_CMD = $(QUIET_LINK) $(MKTGTDIR) ; $(CC) $(EXE_LDFLAGS) $(LDFLAGS) -o $@ -L$(OUT) \
	$(subst $(OUT)/libR2H-PDF.$(SO)$(SO_VERSION),-lR2H-PDF,$^) \
	$(LIBS)
  endif
endif
...
$(OUT)/source/fitz/memento.o : source/fitz/memento.c
	$(CC_CMD) $(WARNING_CFLAGS) $(LIB_CFLAGS) $(THIRD_CFLAGS) -DMEMENTO_R2H-PDF_HACKS
...
R2H-PDF_SRC := $(sort $(wildcard source/fitz/*.c))
R2H-PDF_SRC += $(sort $(wildcard source/fitz/*.cpp))
R2H-PDF_SRC += $(sort $(wildcard source/pdf/*.c))
ifneq ($(xps),no)
R2H-PDF_SRC += $(sort $(wildcard source/xps/*.c))
endif
ifneq ($(svg),no)
R2H-PDF_SRC += $(sort $(wildcard source/svg/*.c))
endif
ifneq ($(html),no)
R2H-PDF_SRC += $(sort $(wildcard source/html/*.c))
endif
R2H-PDF_SRC += $(sort $(wildcard source/reflow/*.c))
R2H-PDF_SRC += $(sort $(wildcard source/cbz/*.c))

R2H-PDF_OBJ := $(R2H-PDF_SRC:%.c=$(OUT)/%.o)
R2H-PDF_OBJ := $(R2H-PDF_OBJ:%.cpp=$(OUT)/%.o)
...
ifeq ($(HAVE_OBJCOPY),yes)
  R2H-PDF_OBJ += $(HYPH_BIN:%.zip=$(OUT)/%.zip.o)
  $(OUT)/%.zip.o : %.zip ; $(OBJCOPY_CMD)
else
  R2H-PDF_OBJ += $(HYPH_GEN:%.c=$(OUT)/%.o)
endif
...
ifeq ($(shared),yes)
  $(OUT)/libR2H-PDF.$(SO)$(SO_VERSION): $(R2H-PDF_OBJ) $(THIRD_OBJ)
	$(LINK_SO_CMD) $(THIRD_LIBS) $(LIBCRYPTO_LIBS) $(LIBS)
  ifeq ($(OS),OpenBSD)
    # should never create symlink
    R2H-PDF_LIB = $(OUT)/libR2H-PDF.$(SO)$(SO_VERSION)
  else
    R2H-PDF_LIB = $(OUT)/libR2H-PDF.$(SO)
    ifneq ($(SO_VERSION),)
      # create symlink with soname if needed
      $(OUT)/libR2H-PDF.$(SO): $(OUT)/libR2H-PDF.$(SO)$(SO_VERSION)
	$(SYMLINK_CMD) $(notdir $<) $@
    endif
  endif
else
  R2H-PDF_LIB = $(OUT)/libR2H-PDF.a
  ifneq ($(THIRD_OBJ),)
    # omit libR2H-PDF-third if there is nothing in it
    THIRD_LIB = $(OUT)/libR2H-PDF-third.a
  endif
  $(R2H-PDF_LIB) : $(R2H-PDF_OBJ)
  $(THIRD_LIB) : $(THIRD_OBJ)
endif

ifneq ($(USE_SYSTEM_GLUT),yes)
  THIRD_GLUT_LIB = $(OUT)/libR2H-PDF-glut.a
  $(THIRD_GLUT_LIB) : $(THIRD_GLUT_OBJ)
endif

THREAD_LIB = $(OUT)/libR2H-PDF-threads.a
$(THREAD_LIB) : $(THREAD_OBJ)

PKCS7_LIB = $(OUT)/libR2H-PDF-pkcs7.a
$(PKCS7_LIB) : $(PKCS7_OBJ)
...
$(MUTOOL_EXE) : $(MUTOOL_OBJ) $(R2H-PDF_LIB) $(THIRD_LIB) $(PKCS7_LIB) $(THREAD_LIB)
...
$(MURASTER_EXE) : $(MURASTER_OBJ) $(R2H-PDF_LIB) $(THIRD_LIB) $(PKCS7_LIB) $(THREAD_LIB)
	$(LINK_CMD) $(THIRD_LIBS) $(THREADING_LIBS) $(LIBCRYPTO_LIBS)
...
ifeq ($(HAVE_GLUT),yes)
  ...
  MUVIEW_GLUT_EXE := $(OUT)/R2H-PDF-gl
  $(MUVIEW_GLUT_EXE) : $(MUVIEW_GLUT_OBJ) $(R2H-PDF_LIB) $(THIRD_LIB) $(THIRD_GLUT_LIB) $(PKCS7_LIB)
	$(LINK_CMD) $(THIRD_LIBS) $(LIBCRYPTO_LIBS) $(THIRD_GLUT_LIBS)
  VIEW_APPS += $(MUVIEW_GLUT_EXE)
endif

ifeq ($(HAVE_X11),yes)
  MUVIEW_X11_EXE := $(OUT)/R2H-PDF-x11
  ...
  $(MUVIEW_X11_EXE) : $(MUVIEW_X11_OBJ) $(R2H-PDF_LIB) $(THIRD_LIB) $(PKCS7_LIB)
...
ifeq ($(HAVE_CURL),yes)
ifeq ($(HAVE_PTHREAD),yes)
  MUVIEW_X11_CURL_EXE := $(OUT)/R2H-PDF-x11-curl
  ...
  $(MUVIEW_X11_CURL_EXE) : $(MUVIEW_X11_CURL_OBJ) $(R2H-PDF_LIB) $(THIRD_LIB) $(PKCS7_LIB) $(CURL_LIB)
...
$(OUT)/example: docs/examples/example.c $(R2H-PDF_LIB) $(THIRD_LIB)
	$(LINK_CMD) $(CFLAGS) $(THIRD_LIBS)
$(OUT)/multi-threaded: docs/examples/multi-threaded.c $(R2H-PDF_LIB) $(THIRD_LIB)
	$(LINK_CMD) $(CFLAGS) $(THIRD_LIBS) -lpthread
$(OUT)/storytest: docs/examples/storytest.c $(R2H-PDF_LIB) $(THIRD_LIB)
	$(LINK_CMD) $(CFLAGS) $(THIRD_LIBS)
$(OUT)/searchtest: docs/examples/searchtest.c $(R2H-PDF_LIB) $(THIRD_LIB)
	$(LINK_CMD) $(CFLAGS) $(THIRD_LIBS)
...
docdir ?= $(prefix)/share/doc/R2H-PDF
pydir ?= $(shell python3 -c "import sysconfig; print(sysconfig.get_path('platlib'))")
SO_INSTALL_MODE ?= 644

third: $(THIRD_LIB)
extra-libs: $(THIRD_GLUT_LIB)
libs: $(R2H-PDF_LIB) $(THIRD_LIB) $(COMMERCIAL_LIBS)
commercial-libs: $(COMMERCIAL_LIBS)
tools: $(TOOL_APPS)
apps: $(TOOL_APPS) $(VIEW_APPS)
extra-apps: $(EXTRA_TOOL_APPS) $(EXTRA_VIEW_APPS)
libR2H-PDF-threads: $(THREAD_LIB)

install-headers:
	install -d $(DESTDIR)$(incdir)/R2H-PDF
	install -d $(DESTDIR)$(incdir)/R2H-PDF/fitz
	install -d $(DESTDIR)$(incdir)/R2H-PDF/pdf
	install -m 644 include/R2H-PDF/*.h $(DESTDIR)$(incdir)/R2H-PDF
	install -m 644 include/R2H-PDF/fitz/*.h $(DESTDIR)$(incdir)/R2H-PDF/fitz
	install -m 644 include/R2H-PDF/pdf/*.h $(DESTDIR)$(incdir)/R2H-PDF/pdf

install-libs: libs install-headers
	install -d $(DESTDIR)$(libdir)
ifeq ($(shared),yes)
	install -m $(SO_INSTALL_MODE) $(OUT)/libR2H-PDF.$(SO)$(SO_VERSION) $(DESTDIR)$(libdir)/libR2H-PDF.$(SO)$(SO_VERSION)
  ifneq ($(OS),OpenBSD)
	ln -sf libR2H-PDF.$(SO)$(SO_VERSION) $(DESTDIR)$(libdir)/libR2H-PDF.$(SO)$(SO_VERSION_MAJOR)
	ln -sf libR2H-PDF.$(SO)$(SO_VERSION) $(DESTDIR)$(libdir)/libR2H-PDF.$(SO)
  endif
else
	install -m 644 $(R2H-PDF_LIB) $(DESTDIR)$(libdir)
...
VENV_FLAG ?= --venv
c++-%: shared-%
	./scripts/R2H-PDFwrap.py $(VENV_FLAG) -d $(OUT) -b 01
python-%: c++-%
	./scripts/R2H-PDFwrap.py $(VENV_FLAG) -d $(OUT) -b 23
csharp-%: c++-%
	./scripts/R2H-PDFwrap.py $(VENV_FLAG) -d $(OUT) -b --csharp 23
...
install-shared-c++: install-shared-c c++
	install -m 644 platform/c++/include/R2H-PDF/*.h $(DESTDIR)$(incdir)/R2H-PDF
	install -m $(SO_INSTALL_MODE) $(OUT)/libR2H-PDFcpp.$(SO)$(SO_VERSION) $(DESTDIR)$(libdir)/
ifneq ($(OS),OpenBSD)
	ln -sf libR2H-PDFcpp.$(SO)$(SO_VERSION) $(DESTDIR)$(libdir)/libR2H-PDFcpp.$(SO)
	ln -sf libR2H-PDFcpp.$(SO)$(SO_VERSION) $(DESTDIR)$(libdir)/libR2H-PDFcpp.$(SO)$(SO_VERSION_MAJOR)
endif

install-shared-python: install-shared-c++ python
	install -d $(DESTDIR)$(pydir)/R2H-PDF
	install -m $(SO_INSTALL_MODE) $(OUT)/_R2H-PDF.$(SO) $(DESTDIR)$(pydir)/R2H-PDF
	install -m 644 $(OUT)/R2H-PDF.py $(DESTDIR)$(pydir)/R2H-PDF/__init__.py
